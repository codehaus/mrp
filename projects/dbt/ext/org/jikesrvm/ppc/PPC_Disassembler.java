 ...
 * @author Ton Ngo
 * 
 * Defined: disasm(inst, addr, buf, reg) INSTRUCTION inst; Address addr; CHAR
 * *buf; CHAR reg[4][10];
 * 
 * 31 Jul 1990 Derek Lieber. Borrowed from libdbx (opcode.c, decode.c).
 * 
 * 30 Jan 1998 Ton Ngo: adapted for Java debugger jdp
 * 
 * 7 Aug 1998 John Whaley: rewritten in Java
 * 
 * 26 Jan 1999 Ton Ngo: New instruction for PowerPC that are not in the POWER
 * instruction set opcode is bit 0-5 of the instruction extop is the extended
 * opcode as listed in the manual key is extop plus additional bits for
 * variation on an opcode (. or o) form is the instruction format: I, B, SC, D,
 * DS, X, XL, XFX, XFL, XO, A, M format is a local enumeration to specify how to
 * print the instruction (this is specific to each decode_?? method).
 * 
 * 18 Dec 2000 Dave Grove: Changed the mnemonics for POWER instructions that
 * have different names on the PowerPC to use the PowerPC mnemonic. (Applied
 * changes from Appendix F of PPC Architecture book).
 * 
 * mnemonic opcode extop key form format Example -------- ------ --- --- ----
 * ------ -------- dcbf 31 86 172 X 6 7C0218AC dcbi 31 470 940 X 6 7C021BAC
 * dcbst 31 54 108 X 6 7C02186C dcbt 31 278 556 X 6 7C021A2C dcbtst 31 246 492 X
 * 6 7C0219EC dcbz 31 1014 2028 X 6 7C021FEC divw 31 491 982 XO 1 7C221BD6 divw.
 * 31 491 983 XO 1 7C221BD7 divwo 31 491 2006 XO 1 7C221FD6 divwo. 31 491 2007
 * XO 1 7C221FD7 divwu 31 459 918 XO 1 7C221B96 divwu. 31 459 919 XO 1 7C221B97
 * divwuo 31 459 1942 XO 1 7C221F96 divwuo. 31 459 1943 XO 1 7C221F97 eieio 31
 * 854 1708 X 1 7C0006AC extsb 31 954 1908 X 0 7C220774 extsb. 31 954 1909 X 0
 * 7C220775 icbi 31 982 1964 X 6 7C021FAC lwarx 31 20 40 X 7 7C221828 mfsrin 31
 * 659 1318 X 4 7C201526 mulhw 31 75 150 XO 1 7C221896 mulhw. 31 75 151 XO 1
 * 7C221897 mulhwu 31 11 22 XO 1 7C221816 mulhwu. 31 11 23 XO 1 7C221817 stwcx.
 * 31 150 301 X 7 7C22192D subf 31 40 80 XO 1 7C221850 subf. 31 40 81 XO 1
 * 7C221851 subfo 31 40 1104 XO 1 7C221C50 subfo. 31 40 1105 XO 1 7C221C51
 * 
 * fadds 59 21 42 A 0 EC22182A fadds. 59 21 43 A 0 EC22182B fdivs 59 18 36 A 0
 * EC221824 fdivs. 59 18 37 A 0 EC221825 fmadds 59 29 58 A 2 EC22193A fmadds. 59
 * 29 59 A 2 EC22193B fmsubs 59 28 56 A 2 EC221938 fmsubs. 59 28 57 A 2 EC221939
 * fmuls 59 25 50 A 1 EC2200F2 fmuls. 59 25 51 A 1 EC2200F3 fnmadds 59 31 62 A 2
 * EC22193E fnmadds. 59 31 63 A 2 EC22193F fnmsubs 59 30 60 A 2 EC22193C
 * fnmsubs. 59 30 61 A 2 EC22193D fsubs 59 20 40 A 0 EC221828 fsubs. 59 20 41 A
 * 0 EC221829 mfear mfpvw mfsprg mtear mtsprg mfdbatl mfdbatu mtdbatl mtdbatu
 * mttb mttbu mftb mftbu mfibatl mfibatu mtibatl mtibatu
 * 
 * 23 Apr 2003 Kris Venstermans: Added instruction decoding for 64 bit
 * architecture. Use of constant definitions instead of integers. Cleaned up
 * some old power instructions.
 */
...
public class PPC_Disassembler {
  ...
  static final String GPR_NAMES[] = { "R0", "R1", "R2", "R3", "R4", "R5", "R6",
      "R7", "R8", "R9", "R10", "R11", "R12", "R13", "R14", "R15", "R16", "R17",
      "R18", "R19", "R20", "R21", "R22", "R23", "R24", "R25", "R26", "R27",
      "R28", "R29", "R30", "R31" };

  static final String FPR_NAMES[] = { "F0", "F1", "F2", "F3", "F4", "F5", "F6",
  ...
  /**
   * Opcode 30 table: The key is bits 27 through 30 of the instruction. "Form"
   * is the instruction format: I, B, SC, D, DS, X, XL, XFX, XFL, XO, A, M, MDS,
   * MD "format" is how the instruction should be printed (specific to the
   * disassembler)
   */
  ...
  /* --- ---- ------ -------- */
  new opcodeXX(0, MD_FORM, 0, "rldicl"), new opcodeXX(1, MD_FORM, 0, "rldicl"),
  ...
  /**
   * Opcode 31 table: The key is bits 21 through 31 of the instruction. "Form"
   * is the instruction format: I, B, SC, D, DS, X, XL, XFX, XFL, XO, A, M
   * "format" is how the instruction should be printed (specific to the
   * disassembler)
   */
  ...
  static opcodeXX[] opcode31 = {
      ...
      new opcodeXX(42, X_FORM, 7, "ldx"), new opcodeXX(43, X_FORM, 7, "ldx."),
      new opcodeXX(54, X_FORM, 8, "sld"), new opcodeXX(55, X_FORM, 8, "sld."),
  ...
  /*
   * Opcode 63 table: The key is computed by taking bits 21 through 31 of the
   * instruction. "Form" is the instruction format and "format" is how the
  ...
  /*
   * opcode59 table: These are the addition for the PowerPC set Key is bits 26
   * through 31 of the instruction. "Form" is the instruction format and
   * "format" is how the instruction should be printed (the enumeration is
   * specific to each decode method)
   */

  /* key form format mnemonic */
  new opcodeXX(42, A_FORM, 0, "fadds"), new opcodeXX(43, A_FORM, 0, "fadds."),
  ...
  /*
   * Aform table: The key is computed by taking bits 26 through 31 of the
   * instruction. "Form" is the instruction format and "format" is how the
   * instruction should be printed.
   */

  /* key form format mnemonic */
  new opcodeXX(36, A_FORM, 0, "fdiv"), new opcodeXX(37, A_FORM, 0, "fdiv."),
      new opcodeXX(40, A_FORM, 0, "fsub"),
      new opcodeXX(41, A_FORM, 0, "fsub."),
      new opcodeXX(42, A_FORM, 0, "fadd"),
      new opcodeXX(43, A_FORM, 0, "fadd."), new opcodeXX(50, A_FORM, 1, "fm"),
      new opcodeXX(51, A_FORM, 1, "fm."), new opcodeXX(56, A_FORM, 2, "fmsub"),
  ...
  /*
   * Translate an instruction from its numeric form into something more
   * readable.
   */
  ...
  static String decode_Dform(int inst, int opcode) {
    ...
    case 3: /* Trap immediate */
      ...
      if (common_opt != null) {
        ...
        return "        ".substring(mnemonic.length()) + mnemonic + "   " + TO
            + "," + rname(RA) + "," + datafield;
      }
    case 4:
  ...
  static String[] Mforms = { "rlimi", "rlimi.", "rlinm", "rlinm.", "rlmi",
      "rlmi.", "rlnm", "rlnm." };
    ...
    if ((opcode < 20) || ((opcode >>> 23) != 0)) {
      ...
      if (opcode == 21) { /* sri and sli are special forms of rlmni */
        if ((ME == 32) && (MB == (31 - SH_RB))) {
          ...
          return "        ".substring(asm_mnemonic.length()) + asm_mnemonic
              + "   " + rname(RA) + "," + rname(RS) + ","
              + intAsHexString(SH_RB) + "," + intAsHexString(MB) + ","
              + intAsHexString(ME);
        }
      } else {
        return "        ".substring(asm_mnemonic.length()) + asm_mnemonic
  ...
  static String decode_MDform(int inst, String mnemonic) {
    ...
    return "        ".substring(mnemonic.length()) + mnemonic + rname(RA)
        + ", " + rname(RS) + ", " + SH + ", " + MB;
  }
  ...
  static String decode_Xform(int inst, String mnemonic, int format, int ext_op) {
    ...
    case 8:
      if ((ext_op == 888) || (ext_op == 889)) {
        if (rt == RB) {
          ...
          return "        ".substring(mne.length()) + mne + "   " + rname(RA)
              + "," + rname(rt);
        }
      }
      return "        ".substring(mnemonic.length()) + mnemonic + "   "
    ...
    case 12:
      return "        ".substring(mnemonic.length()) + mnemonic + "   fr" + FRS
          + "," + rname(RA) + "," + rname(RB);
    case 13:
      return "        ".substring(mnemonic.length()) + mnemonic + "   fr" + FRT;
    case 14:
  ...
  static String build_branch_op(int br_opt, int cr_bit, int update_link,
      int absolute, int ext_op) {
    String c;
  ...
  /*
   * Return whether an instruction is a branch for yieldpoint used by OPT
   * compiler
   */
