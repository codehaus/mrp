...
public class PPC_InstructionDecoder implements
   ...
   * 
   * @param x
   *          value to return range of bits from
   * @param n
   *          the start of the range of bits
   * @param m
   *          the end of the range of bits
  ...
  /**
   * Give the decoder for a given instruction. Extended instruction decoders
   * should return the appropriate instruction from opcode 2.
   * 
   * @param the
   *          instruction to get the decoder for
   * @param the
   *          appropriate instruction decoder
   */
   ...
   * 
   * @param ps
   *          the process space of the interpretation
  ...
  /**
   * Following interpreting an instruction, move the program counter along, set
   * the current instruction and find the appropriate decoder.
   */
  ...
  public static int translateInstruction(PPC2IR ppc2ir, PPC_ProcessSpace ps,
    ...
    {
    case D_FORM:
      return opCodeLookUp[primaryOpcode].decoder.translateD_FORM(ppc2ir, lazy,
          pc, instr, primaryOpcode, bits(instr, 6, 10), bits(instr, 11, 15),
          bits(instr, 16, 31));
    case B_FORM:
      return opCodeLookUp[primaryOpcode].decoder.translateB_FORM(ppc2ir, lazy,
          pc, instr, primaryOpcode, bits(instr, 6, 10), bits(instr, 11, 15),
          bits(instr, 16, 29), bits(instr, 30, 30), instr & 0x1);
    case I_FORM:
      return opCodeLookUp[primaryOpcode].decoder.translateI_FORM(ppc2ir, lazy,
          pc, instr, primaryOpcode, bits(instr, 6, 29), bits(instr, 30, 30),
          instr & 0x1);
    case SC_FORM:
      return opCodeLookUp[primaryOpcode].decoder.translateSC_FORM(ppc2ir, lazy,
          pc, instr, primaryOpcode, bits(instr, 6, 10), bits(instr, 11, 15),
          bits(instr, 16, 29), bits(instr, 30, 30), bits(instr, 31, 31));
    case X_FORM:
      return opCodeLookUp[primaryOpcode].decoder.translateX_FORM(ppc2ir, lazy,
          pc, instr, primaryOpcode, bits(instr, 6, 10), bits(instr, 11, 15),
          bits(instr, 16, 20), bits(instr, 21, 30), instr & 0x1);
    case XFX_FORM:
      return opCodeLookUp[primaryOpcode].decoder.translateXFX_FORM(ppc2ir,
          lazy, pc, instr, primaryOpcode, bits(instr, 6, 10), bits(instr, 11,
              20), bits(instr, 21, 30), instr & 0x1);
    case XFL_FORM:
      return opCodeLookUp[primaryOpcode].decoder.translateXFL_FORM(ppc2ir,
          lazy, pc, instr, primaryOpcode, bits(instr, 6, 6),
          bits(instr, 7, 14), bits(instr, 15, 15), bits(instr, 16, 20), bits(
              instr, 21, 30), instr & 0x1);
    case XL_FORM:
      return opCodeLookUp[primaryOpcode].decoder.translateXL_FORM(ppc2ir, lazy,
          pc, instr, primaryOpcode, bits(instr, 6, 10), bits(instr, 11, 15),
          bits(instr, 16, 20), bits(instr, 21, 30), instr & 0x1);
    case XO_FORM:
      return opCodeLookUp[primaryOpcode].decoder.translateXO_FORM(ppc2ir, lazy,
          pc, instr, primaryOpcode, bits(instr, 6, 10), bits(instr, 11, 15),
          bits(instr, 16, 20), bits(instr, 21, 21), bits(instr, 22, 30),
          instr & 0x1);
    case M_FORM:
      return opCodeLookUp[primaryOpcode].decoder.translateM_FORM(ppc2ir, lazy,
          pc, instr, primaryOpcode, bits(instr, 6, 10), bits(instr, 11, 15),
          bits(instr, 16, 20), bits(instr, 21, 25), bits(instr, 26, 30),
          instr & 0x1);
    case A_FORM:
      return opCodeLookUp[primaryOpcode].decoder.translateA_FORM(ppc2ir, lazy,
          pc, instr, primaryOpcode, bits(instr, 6, 10), bits(instr, 11, 15),
          bits(instr, 16, 20), bits(instr, 21, 25), bits(instr, 26, 30),
          instr & 0x1);
    case EXTENDED:
      return opCodeLookUp[primaryOpcode].decoder.translateEXTENDED(ppc2ir,
          lazy, pc, instr, primaryOpcode);
  ...
  protected int translateD_FORM(PPC2IR ppc2ir, PPC_Laziness lazy, int pc,
      int inst, int opcode, int bits6_to_10, int bits11_to_15, int bits16_to_31) {
    throw new Error(
        "Attempting to translateD_Form on a non D_Form instruction\n"
            + "Address=0x" + Integer.toHexString(pc) + " Instruction 0x"
  ...
  protected int translateDS_FORM(int inst, int opcode) {
    throw new Error(
        "Attempting to translateDS_Form on a non DS_Form instruction\n"
            + "Instruction 0x" + Integer.toHexString(inst) + "\n" + "Opcode 0x"
            + Integer.toHexString(inst));
  }

  protected int translateB_FORM(PPC2IR ppc2ir, PPC_Laziness lazy, int pc,
      int inst, int primaryOpcode, int bits_6_to_10, int bits_11_to_15,
      int bits16_to_29, int bit30, int bit31) {
    throw new Error(
  ...
  protected int translateI_FORM(PPC2IR ppc2ir, PPC_Laziness lazy, int pc,
      int inst, int opcode, int bits_6_to_29, int bit30, int bit31) {
    throw new Error(
  ...
  protected int translateSC_FORM(PPC2IR ppc2ir, PPC_Laziness lazy, int pc,
      int inst, int primaryOpcode, int bits_6_to_10, int bits_11_to_15,
      int bits_16_to_29, int bit30, int bit31) {
    throw new Error(
        "Attempting to translateSC_Form on a non SC_Form instruction\n"
            + "Instruction 0x" + Integer.toHexString(inst) + "\n" + "Opcode 0x"
  ...
  protected int translateX_FORM(PPC2IR ppc2ir, PPC_Laziness lazy, int pc,
      int inst, int opcode, int bits6_to_10, int bits11_to_15,
      int bits16_to_20, int secondaryOpcode, int bit31) {
    throw new Error(
        "Attempting to translateX_Form on a non X_Form instruction\n"
            + "Address=0x" + Integer.toHexString(pc) + " Instruction 0x"
            + Integer.toHexString(inst) + "\n" + "Opcode 0x"
            + Integer.toHexString(opcode) + " bits6_to_10=" + bits6_to_10
  ...
  protected int translateXFX_FORM(PPC2IR ppc2ir, PPC_Laziness lazy, int pc,
      int inst, int opcode, int bits6_to_10, int bits11_to_20,
      int secondaryOpcode, int bit31) {
    throw new Error(
        "Attempting to translateXFX_Form on a non XFX_Form instruction\n"
            + "Address=0x" + Integer.toHexString(pc) + " Instruction 0x"
            + Integer.toHexString(inst) + "\n" + "Opcode 0x"
            + Integer.toHexString(opcode) + " bits6_to_10=" + bits6_to_10
  ...
  protected int translateXFL_FORM(PPC2IR ppc2ir, PPC_Laziness lazy, int pc,
      int inst, int opcode, int bit6, int bits7_to_14, int bit15,
      int bits16_to_20, int secondaryOpcode, int bit31) {
    throw new Error(
        "Attempting to translateXFL_Form on a non XFL_Form instruction\n"
            + "Address=0x" + Integer.toHexString(pc) + " Instruction 0x"
            + Integer.toHexString(inst) + "\n" + "Opcode 0x"
  ...
  protected int translateXL_FORM(PPC2IR ppc2ir, PPC_Laziness lazy, int pc,
      ...
      int bits16_to_20, int secondaryOpcode, int bit31) {
    throw new Error(
        "Attempting to translateXL_Form on a non XL_Form instruction\n"
            + "Address=0x" + Integer.toHexString(pc) + " Instruction 0x"
            + Integer.toHexString(inst) + "\n" + "Opcode 0x"
  ...
  protected int translateXO_FORM(PPC2IR ppc2ir, PPC_Laziness lazy, int pc,
      int inst, int opcode, int bits6_to_10, int bits11_to_15,
      int bits16_to_20, int bit21, int secondaryOpcode, int bit31) {
    throw new Error(
        "Attempting to translateXO_Form on a non XO_Form instruction\n"
            + "Address=0x" + Integer.toHexString(pc) + " Instruction 0x"
            + Integer.toHexString(inst) + "\n" + "Opcode 0x"
            + Integer.toHexString(inst) + " bits6_to_10=" + bits6_to_10
            + " bits11_to_15=" + bits11_to_15 + " bits16_to_20=" + bits16_to_20
  ...
  protected int translateM_FORM(PPC2IR ppc2ir, PPC_Laziness lazy, int pc,
      int inst, int opcode, int bits6_to_10, int bits11_to_15,
      int bits16_to_20, int bits21_to_25, int bits26_to_30, int bit31) {
    throw new Error(
        "Attempting to translateM_Form on a non M_Form instruction\n"
            + "Address=0x" + Integer.toHexString(pc) + " Instruction 0x"
            + Integer.toHexString(inst) + "\n" + "Opcode 0x"
            + Integer.toHexString(inst) + " bits6_to_10=" + bits6_to_10
            + " bits11_to_15=" + bits11_to_15 + " bits16_to_20=" + bits16_to_20
  ...
  protected OpCodeLookUp getOpCodeLookUp(int primaryOpcode, int secondaryOpcode) {
    throw new Error("Unknown opcode in opcode look up. Primary opcode "
        + primaryOpcode + " secondary opcode " + secondaryOpcode);
  }

  protected int translateEXTENDED(PPC2IR ppc2ir, PPC_Laziness lazy, int pc,
      int instr, int primaryOpcode) {
    ...
    {
    case D_FORM:
      return opCodeLookUp.decoder.translateD_FORM(ppc2ir, lazy, pc, instr,
          primaryOpcode, bits(instr, 6, 10), bits(instr, 11, 15), bits(instr,
              16, 31));
    case B_FORM:
      return opCodeLookUp.decoder.translateB_FORM(ppc2ir, lazy, pc, instr,
          primaryOpcode, bits(instr, 6, 10), bits(instr, 11, 15), bits(instr,
              16, 29), bits(instr, 30, 30), instr & 0x1);
    case I_FORM:
      return opCodeLookUp.decoder.translateI_FORM(ppc2ir, lazy, pc, instr,
          primaryOpcode, bits(instr, 6, 29), bits(instr, 30, 30), instr & 0x1);
    case SC_FORM:
      return opCodeLookUp.decoder.translateSC_FORM(ppc2ir, lazy, pc, instr,
          primaryOpcode, bits(instr, 6, 10), bits(instr, 11, 15), bits(instr,
              16, 29), bits(instr, 30, 30), instr & 0x1);
    case X_FORM:
      return opCodeLookUp.decoder.translateX_FORM(ppc2ir, lazy, pc, instr,
          primaryOpcode, bits(instr, 6, 10), bits(instr, 11, 15), bits(instr,
              16, 20), secondaryOpcode, instr & 0x1);
    case XFX_FORM:
      return opCodeLookUp.decoder.translateXFX_FORM(ppc2ir, lazy, pc, instr,
          primaryOpcode, bits(instr, 6, 10), bits(instr, 11, 20),
          secondaryOpcode, instr & 0x1);
    case XFL_FORM:
      return opCodeLookUp.decoder.translateXFL_FORM(ppc2ir, lazy, pc, instr,
          primaryOpcode, bits(instr, 6, 6), bits(instr, 7, 14), bits(instr, 15,
              15), bits(instr, 16, 20), secondaryOpcode, instr & 0x1);
    case XL_FORM:
      return opCodeLookUp.decoder.translateXL_FORM(ppc2ir, lazy, pc, instr,
          primaryOpcode, bits(instr, 6, 10), bits(instr, 11, 15), bits(instr,
              16, 20), secondaryOpcode, instr & 0x1);
    case XO_FORM:
      return opCodeLookUp.decoder.translateXO_FORM(ppc2ir, lazy, pc, instr,
          primaryOpcode, bits(instr, 6, 10), bits(instr, 11, 15), bits(instr,
              16, 20), bits(instr, 21, 21), bits(instr, 22, 30), instr & 0x1);
    case M_FORM:
      return opCodeLookUp.decoder.translateM_FORM(ppc2ir, lazy, pc, instr,
          primaryOpcode, bits(instr, 6, 10), bits(instr, 11, 15), bits(instr,
              16, 20), bits(instr, 21, 25), bits(instr, 26, 30), instr & 0x1);
    case A_FORM:
      return opCodeLookUp.decoder.translateA_FORM(ppc2ir, lazy, pc, instr,
          primaryOpcode, bits(instr, 6, 10), bits(instr, 11, 15), bits(instr,
              16, 20), bits(instr, 21, 25), bits(instr, 26, 30), instr & 0x1);
    case EXTENDED:
      return opCodeLookUp.decoder.translateEXTENDED(ppc2ir, lazy, pc, instr,
          primaryOpcode);
    case INVALID_OP: /* More work to do... */
    default:
      throw new Error("Invalid opcode. primary " + primaryOpcode
   ...
   * 
   * @param value
   *          a signed number occupying &lt;32 bits.
   * @param n
   *          the number of digits used in the original value.
   */
  ...
  protected static void plantDecrementCTR(PPC2IR ppc2ir) {
    RegisterOperand ctr = ppc2ir.getCTRRegister();
    ppc2ir.appendInstruction(Binary.create(INT_SUB, ctr, ctr
        .copyRO(), new IntConstantOperand(1)));
  }
   ...
   * @param lazy
   *          the current lazy state
   * @param crf
   *          the field to change
   * @param rA
   *          the lhs operand for the compare
   * @param rB
   *          the rhs operand for the compare
   * @param compareType
   *          whether to do a signed or unsigned comparison on rA and rB
   */
  protected static void setCRfield(PPC2IR ppc2ir, PPC_Laziness lazy, int crf,
      Operand rA, Operand rB, int compareType) {
    RegisterOperand lt = ppc2ir.getCR_Lt_Register(crf);
    RegisterOperand gt = ppc2ir.getCR_Gt_Register(crf);
    RegisterOperand eq = ppc2ir.getCR_Eq_Register(crf);
    RegisterOperand so = ppc2ir.getCR_SO_Register(crf);

    switch (compareType) {
    case SIGNED_INT_CMP:
      ppc2ir.appendInstruction(BooleanCmp.create(BOOLEAN_CMP_INT,
          lt, rA, rB, ConditionOperand.LESS(), BranchProfileOperand
              .unlikely()));
      ppc2ir.appendInstruction(BooleanCmp.create(BOOLEAN_CMP_INT,
          gt, rA.copy(), rB.copy(), ConditionOperand.GREATER(),
          BranchProfileOperand.unlikely()));
      ppc2ir.appendInstruction(BooleanCmp.create(BOOLEAN_CMP_INT,
          eq, rA.copy(), rB.copy(), ConditionOperand.EQUAL(),
          BranchProfileOperand.unlikely()));
    ...
    case UNSIGNED_INT_CMP:
      ppc2ir.appendInstruction(BooleanCmp.create(BOOLEAN_CMP_INT,
          lt, rA, rB, ConditionOperand.LOWER(), BranchProfileOperand
              .unlikely()));
      ppc2ir.appendInstruction(BooleanCmp.create(BOOLEAN_CMP_INT,
          gt, rA.copy(), rB.copy(), ConditionOperand.HIGHER(),
          BranchProfileOperand.unlikely()));
      ppc2ir.appendInstruction(BooleanCmp.create(BOOLEAN_CMP_INT,
          eq, rA.copy(), rB.copy(), ConditionOperand.EQUAL(),
          BranchProfileOperand.unlikely()));
    ...
    case FP_CMPU:
      ppc2ir.appendInstruction(BooleanCmp.create(BOOLEAN_CMP_INT,
          lt, rA, rB, ConditionOperand.CMPG_LESS(),
          BranchProfileOperand.unlikely()));
      ppc2ir.appendInstruction(BooleanCmp.create(BOOLEAN_CMP_INT,
          gt, rA.copy(), rB.copy(), ConditionOperand.CMPL_GREATER(),
          BranchProfileOperand.unlikely()));
      ppc2ir.appendInstruction(BooleanCmp.create(BOOLEAN_CMP_INT,
          eq, rA.copy(), rB.copy(), ConditionOperand.CMPL_EQUAL(),
          BranchProfileOperand.unlikely()));
      ppc2ir.appendInstruction(BooleanCmp2.create(
          BOOLEAN_CMP2_INT_OR, so, rA.copy(), rA.copy(), ConditionOperand
              .CMPL_NOT_EQUAL(), BranchProfileOperand.unlikely(),
          rB.copy(), rB.copy(), ConditionOperand.CMPL_NOT_EQUAL(),
          BranchProfileOperand.unlikely()));
      break;
    default:
  ...
  /**
   * Plant (likely) branch to block if ctr is zero (ctr_zero == true) or not
   * zero (ctr_zero == false). NB as this code is likely to be used to branch to
   * a fall through case the likely and ctr_zero values are usually the opposite
   * of what the instruction encodes
   */
  protected static void plantBranchToBlockDependentOnCTR(PPC2IR ppc2ir,
      BasicBlock targetBlock, boolean ctr_zero, boolean likely) {
    RegisterOperand ctr = ppc2ir.getCTRRegister();
    RegisterOperand guardResult = ppc2ir.getTempValidation(0);
    ConditionOperand condOp;
    if (ctr_zero) {
      condOp = ConditionOperand.EQUAL();
    } else {
      condOp = ConditionOperand.NOT_EQUAL();
    }
    BranchProfileOperand likelyOp = ppc2ir
        .getConditionalBranchProfileOperand(likely);

    BasicBlock fallThrough = ppc2ir.createBlockAfterCurrent();
    ppc2ir.appendInstruction(IfCmp.create(INT_IFCMP, guardResult,
        ctr, new IntConstantOperand(0), condOp, targetBlock
  ...
  protected static void plantBranchToBlockDependentOnCondition(PPC2IR ppc2ir,
      BasicBlock targetBlock, PPC_Laziness lazy, int BI, boolean cond_true,
      boolean likely) {
    ...
    RegisterOperand flag = ppc2ir.getCRB_Register(BI);
    ...
    RegisterOperand guardResult = ppc2ir.getTempValidation(0);
    ppc2ir.appendInstruction(IfCmp.create(INT_IFCMP, guardResult,
        flag, new IntConstantOperand(0), condOp, targetBlock
            .makeJumpTarget(), likelyOp));
    ppc2ir.getCurrentBlock().insertOut(targetBlock);
...
final class xlForm_decoder extends PPC_InstructionDecoder {
  ...
  private static final OpCodeLookUp[] XLform = {
      ...
      /* ---- -- ---- -------- ---------- */
      /* 19, 0, */new OpCodeLookUp(XL_FORM, "mcrf",
          new mcrf_decoder()),
      /* 19, 16, */new OpCodeLookUp(XL_FORM, "bclr or bclrl",
          new bclr_decoder()),
      /* 19, 33, */new OpCodeLookUp(XL_FORM, "crnor",
          new crnor_decoder()),
      /* 19, 50, */new OpCodeLookUp(XL_FORM, "rfi",
          new rfi_decoder()),
      /* 19, XXX, */null,
      ...
      /* 19, XXX, */null,
      /* 19, XXX, */null,
      /* 19, 129, */new OpCodeLookUp(XL_FORM, "crandc",
          new crandc_decoder()),
      /* 19, 150, */new OpCodeLookUp(XL_FORM, "isync",
          new isync_transltor()),

      /* 19, XXX, */null,
      /* 19, XXX, */null,
      /* 19, 193, */new OpCodeLookUp(XL_FORM, "crxor",
          new crxor_decoder()),
      /* 19, XXX, */null,
      /* 19, 225, */new OpCodeLookUp(XL_FORM, "crnand",
          new crnand_decoder()),

      /* 19, XXX, */null,
      /* 19, 257, */new OpCodeLookUp(XL_FORM, "crand",
          new crand_decoder()),
      /* 19, XXX, */null,
      /* 19, 289, */new OpCodeLookUp(XL_FORM, "creqv",
          new creqv_decoder()),
      /* 19, XXX, */null,
      ...
      /* 19, XXX, */null,
      /* 19, 417, */new OpCodeLookUp(XL_FORM, "crorc",
          new croc_decoder()),
      /* 19, XXX, */null,
      /* 19, 449, */new OpCodeLookUp(XL_FORM, "cror",
          new cror_traslator()),
      /* 19, XXX, */null,
      ...
      /* 19, XXX, */null,
      /* 19, XXX, */null,
      /* 19, 528, */new OpCodeLookUp(XL_FORM, "bcctr or bcctrl",
          new bcctr_decoder()) };
  ...
  protected int translateXL_FORM(PPC2IR ppc2ir, PPC_Laziness lazy, int pc,
      int inst, int opcode, int bits6_to_10, int bits11_to_15,
      int bits16_to_20, int secondaryOpcode, int bit31) {
    return XLform[secondaryOpcode >> 4].decoder.translateXL_FORM(ppc2ir, lazy,
        pc, inst, opcode, bits6_to_10, bits11_to_15, bits16_to_20,
        secondaryOpcode, bit31);
  }
}
...
final class extended31_decoder extends PPC_InstructionDecoder {
  ...
  private static final OpCodeLookUp subfc_lookup = new OpCodeLookUp(XO_FORM,
      "subfc", new subfc_decoder());

  private static final OpCodeLookUp addc_lookup = new OpCodeLookUp(XO_FORM,
      "addc", new addc_decoder());

  private static final OpCodeLookUp mulhwu_lookup = new OpCodeLookUp(XO_FORM,
      "mulhwu", new mulhwu_decoder());

  private static final OpCodeLookUp subf_lookup = new OpCodeLookUp(XO_FORM,
      "subf", new subf_decoder());

  private static final OpCodeLookUp mulhw_lookup = new OpCodeLookUp(XO_FORM,
      "mulhw", new mulhw_decoder());

  private static final OpCodeLookUp neg_lookup = new OpCodeLookUp(XO_FORM,
      "neg", new neg_decoder());

  private static final OpCodeLookUp subfe_lookup = new OpCodeLookUp(XO_FORM,
      "subfe", new subfe_decoder());

  private static final OpCodeLookUp adde_lookup = new OpCodeLookUp(XO_FORM,
      "adde", new adde_decoder());

  private static final OpCodeLookUp subfze_lookup = new OpCodeLookUp(XO_FORM,
      "subfze", new subfze_decoder());

  private static final OpCodeLookUp addze_lookup = new OpCodeLookUp(XO_FORM,
      "addze", new addze_decoder());

  private static final OpCodeLookUp subfme_lookup = new OpCodeLookUp(XO_FORM,
      "subfme", new subfme_decoder());

  private static final OpCodeLookUp addme_lookup = new OpCodeLookUp(XO_FORM,
      "addme", new addme_decoder());

  private static final OpCodeLookUp mullw_lookup = new OpCodeLookUp(XO_FORM,
      "mullw", new mullw_decoder());

  private static final OpCodeLookUp add_lookup = new OpCodeLookUp(XO_FORM,
      "add", new add_decoder());

  private static final OpCodeLookUp divwu_lookup = new OpCodeLookUp(XO_FORM,
      "divwu", new divwu_decoder());

  private static final OpCodeLookUp divw_lookup = new OpCodeLookUp(XO_FORM,
      "divw", new divw_decoder());

  private static final OpCodeLookUp subfco_lookup = new OpCodeLookUp(XO_FORM,
      "subfco", new subfco_decoder());

  private static final OpCodeLookUp addco_lookup = new OpCodeLookUp(XO_FORM,
      "addco", new addco_decoder());

  private static final OpCodeLookUp subfo_lookup = new OpCodeLookUp(XO_FORM,
      "subfo", new subfo_decoder());

  private static final OpCodeLookUp subfeo_lookup = new OpCodeLookUp(XO_FORM,
      "subfeo", new subfeo_decoder());

  private static final OpCodeLookUp addeo_lookup = new OpCodeLookUp(XO_FORM,
      "addeo", new addeo_decoder());

  private static final OpCodeLookUp subfzeo_lookup = new OpCodeLookUp(XO_FORM,
      "subfzeo", new subfzeo_decoder());

  private static final OpCodeLookUp addzeo_lookup = new OpCodeLookUp(XO_FORM,
      "addzeo", new addzeo_decoder());

  private static final OpCodeLookUp subfmeo_lookup = new OpCodeLookUp(XO_FORM,
      "subfmeo", new subfmeo_decoder());

  private static final OpCodeLookUp addmeo_lookup = new OpCodeLookUp(XO_FORM,
      "addmeo", new addmeo_decoder());

  private static final OpCodeLookUp mullwo_lookup = new OpCodeLookUp(XO_FORM,
      "mullwo", new mullwo_decoder());

  private static final OpCodeLookUp nego_lookup = new OpCodeLookUp(XO_FORM,
      "nego", new nego_decoder());

  private static final OpCodeLookUp addo_lookup = new OpCodeLookUp(XO_FORM,
      "addo", new addo_decoder());

  private static final OpCodeLookUp divwuo_lookup = new OpCodeLookUp(XO_FORM,
      "divwuo", new divwuo_decoder());

  private static final OpCodeLookUp divwo_lookup = new OpCodeLookUp(XO_FORM,
      "divwo", new divwo_decoder());

  // The decoders for the XFX_Form instructions
  private static final OpCodeLookUp mtcrf_lookup = new OpCodeLookUp(XFX_FORM,
      "mtcrf", new mtcrf_decoder());

  private static final OpCodeLookUp mfspr_lookup = new OpCodeLookUp(XFX_FORM,
      "mfspr", new mfspr_decoder());

  private static final OpCodeLookUp mtspr_lookup = new OpCodeLookUp(XFX_FORM,
      "mtspr", new mtspr_decoder());

  // The decoders for the X_Form instructions
  private static final OpCodeLookUp cmp_lookup = new OpCodeLookUp(X_FORM,
      "cmp", new cmp_decoder());

  private static final OpCodeLookUp tw_lookup = new OpCodeLookUp(X_FORM, "tw",
      new tw_decoder());

  private static final OpCodeLookUp lvsl_lookup = new OpCodeLookUp(X_FORM,
      "lvsl", new lvsl_decoder());

  private static final OpCodeLookUp lvebx_lookup = new OpCodeLookUp(X_FORM,
      "lvebx", new lvebx_decoder());

  private static final OpCodeLookUp mulhdu_lookup = new OpCodeLookUp(X_FORM,
      "mulhdu", new mulhdu_decoder());

  private static final OpCodeLookUp mfcr_lookup = new OpCodeLookUp(X_FORM,
      "mfcr", new mfcr_decoder());

  private static final OpCodeLookUp lwarx_lookup = new OpCodeLookUp(X_FORM,
      "lwarx", new lwarx_decoder());

  private static final OpCodeLookUp ldx_lookup = new OpCodeLookUp(X_FORM,
      "ldx", new ldx_decoder());

  private static final OpCodeLookUp lwzx_lookup = new OpCodeLookUp(X_FORM,
      "lwzx", new lwzx_decoder());

  private static final OpCodeLookUp slw_lookup = new OpCodeLookUp(X_FORM,
      "slw", new slw_decoder());

  private static final OpCodeLookUp cntlzw_lookup = new OpCodeLookUp(X_FORM,
      "cntlzw", new cntlzw_decoder());

  private static final OpCodeLookUp sld_lookup = new OpCodeLookUp(X_FORM,
      "sld", new sld_decoder());

  private static final OpCodeLookUp and_lookup = new OpCodeLookUp(X_FORM,
      "and", new and_decoder());

  private static final OpCodeLookUp cmpl_lookup = new OpCodeLookUp(X_FORM,
      "cmpl", new cmpl_decoder());

  private static final OpCodeLookUp lvsr_lookup = new OpCodeLookUp(X_FORM,
      "lvsr", new lvsr_decoder());

  private static final OpCodeLookUp lvehx_lookup = new OpCodeLookUp(X_FORM,
      "lvehx", new lvehx_decoder());

  private static final OpCodeLookUp ldux_lookup = new OpCodeLookUp(X_FORM,
      "ldux", new ldux_decoder());

  private static final OpCodeLookUp dcbst_lookup = new OpCodeLookUp(X_FORM,
      "dcbst", new dcbst_decoder());

  private static final OpCodeLookUp lwzux_lookup = new OpCodeLookUp(X_FORM,
      "lwzux", new lwzux_decoder());

  private static final OpCodeLookUp andc_lookup = new OpCodeLookUp(X_FORM,
      "andc", new andc_decoder());

  private static final OpCodeLookUp td_lookup = new OpCodeLookUp(X_FORM, "td",
      new td_decoder());

  private static final OpCodeLookUp mulhd_lookup = new OpCodeLookUp(X_FORM,
      "mulhd", new mulhd_decoder());

  private static final OpCodeLookUp mfmsr_lookup = new OpCodeLookUp(X_FORM,
      "mfmsr", new mfmsr_decoder());

  private static final OpCodeLookUp ldarx_lookup = new OpCodeLookUp(X_FORM,
      "ldarx", new ldarx_decoder());

  private static final OpCodeLookUp dcbf_lookup = new OpCodeLookUp(X_FORM,
      "dcbf", new dcbf_decoder());

  private static final OpCodeLookUp lbzx_lookup = new OpCodeLookUp(X_FORM,
      "lbzx", new lbzx_decoder());

  private static final OpCodeLookUp lbzux_lookup = new OpCodeLookUp(X_FORM,
      "lbzux", new lbzux_decoder());

  private static final OpCodeLookUp nor_lookup = new OpCodeLookUp(X_FORM,
      "nor", new nor_decoder());

  private static final OpCodeLookUp mtmsr_lookup = new OpCodeLookUp(X_FORM,
      "mtmsr", new mtmsr_decoder());

  private static final OpCodeLookUp stdx_lookup = new OpCodeLookUp(X_FORM,
      "stdx", new stdx_decoder());

  private static final OpCodeLookUp stwcx_lookup = new OpCodeLookUp(X_FORM,
      "stwcx", new stwcx_decoder());

  private static final OpCodeLookUp stwx_lookup = new OpCodeLookUp(X_FORM,
      "stwx", new stwx_decoder());

  private static final OpCodeLookUp stdux_lookup = new OpCodeLookUp(X_FORM,
      "stdux", new stdux_decoder());

  private static final OpCodeLookUp stwux_lookup = new OpCodeLookUp(X_FORM,
      "stwux", new stwux_decoder());

  private static final OpCodeLookUp mtsr_lookup = new OpCodeLookUp(X_FORM,
      "mtsr", new mtsr_decoder());

  private static final OpCodeLookUp stdcx_lookup = new OpCodeLookUp(X_FORM,
      "stdcx", new stdcx_decoder());

  private static final OpCodeLookUp stbx_lookup = new OpCodeLookUp(X_FORM,
      "stbx", new stbx_decoder());

  private static final OpCodeLookUp mulld_lookup = new OpCodeLookUp(X_FORM,
      "mulld", new mulld_decoder());

  private static final OpCodeLookUp mtsrin_lookup = new OpCodeLookUp(X_FORM,
      "mtsrin", new mtsrin_decoder());

  private static final OpCodeLookUp dcbtst_lookup = new OpCodeLookUp(X_FORM,
      "dcbtst", new dcbtst_decoder());

  private static final OpCodeLookUp stbux_lookup = new OpCodeLookUp(X_FORM,
      "stbux", new stbux_decoder());

  private static final OpCodeLookUp dcbt_lookup = new OpCodeLookUp(X_FORM,
      "dcbt", new dcbt_decoder());

  private static final OpCodeLookUp lhzx_lookup = new OpCodeLookUp(X_FORM,
      "lhzx", new lhzx_decoder());

  private static final OpCodeLookUp eqv_lookup = new OpCodeLookUp(X_FORM,
      "eqv", new eqv_decoder());

  private static final OpCodeLookUp tlbie_lookup = new OpCodeLookUp(X_FORM,
      "tlbie", new tlbie_decoder());

  private static final OpCodeLookUp eciwx_lookup = new OpCodeLookUp(X_FORM,
      "eciwx", new eciwx_decoder());

  private static final OpCodeLookUp lhzux_lookup = new OpCodeLookUp(X_FORM,
      "lhzux", new lhzux_decoder());

  private static final OpCodeLookUp xor_lookup = new OpCodeLookUp(X_FORM,
      "xor", new xor_decoder());

  private static final OpCodeLookUp lwax_lookup = new OpCodeLookUp(X_FORM,
      "lwax", new lwax_decoder());

  private static final OpCodeLookUp dst_lookup = new OpCodeLookUp(X_FORM,
      "dst", new dst_decoder());

  private static final OpCodeLookUp lhax_lookup = new OpCodeLookUp(X_FORM,
      "lhax", new lhax_decoder());

  private static final OpCodeLookUp lvxl_lookup = new OpCodeLookUp(X_FORM,
      "lvxl", new lvxl_decoder());

  private static final OpCodeLookUp tlbia_lookup = new OpCodeLookUp(X_FORM,
      "tlbia", new tlbia_decoder());

  private static final OpCodeLookUp mftb_lookup = new OpCodeLookUp(X_FORM,
      "mftb", new mftb_decoder());

  private static final OpCodeLookUp lwaux_lookup = new OpCodeLookUp(X_FORM,
      "lwaux", new lwaux_decoder());

  private static final OpCodeLookUp dstst_lookup = new OpCodeLookUp(X_FORM,
      "dstst", new dstst_decoder());

  private static final OpCodeLookUp lhaux_lookup = new OpCodeLookUp(X_FORM,
      "lhaux", new lhaux_decoder());

  private static final OpCodeLookUp sthx_lookup = new OpCodeLookUp(X_FORM,
      "sthx", new sthx_decoder());

  private static final OpCodeLookUp orc_lookup = new OpCodeLookUp(X_FORM,
      "orc", new orc_decoder());

  private static final OpCodeLookUp slbie_lookup = new OpCodeLookUp(X_FORM,
      "slbie", new slbie_decoder());

  private static final OpCodeLookUp ecowx_lookup = new OpCodeLookUp(X_FORM,
      "ecowx", new ecowx_decoder());

  private static final OpCodeLookUp sthux_lookup = new OpCodeLookUp(X_FORM,
      "sthux", new sthux_decoder());

  private static final OpCodeLookUp or_lookup = new OpCodeLookUp(X_FORM, "or",
      new or_decoder());

  private static final OpCodeLookUp divdu_lookup = new OpCodeLookUp(X_FORM,
      "divdu", new divdu_decoder());

  private static final OpCodeLookUp dcbi_lookup = new OpCodeLookUp(X_FORM,
      "dcbi", new dcbi_decoder());

  private static final OpCodeLookUp nand_lookup = new OpCodeLookUp(X_FORM,
      "nand", new nand_decoder());

  private static final OpCodeLookUp divd_lookup = new OpCodeLookUp(X_FORM,
      "divd", new divd_decoder());

  private static final OpCodeLookUp slbia_lookup = new OpCodeLookUp(X_FORM,
      "slbia", new slbia_decoder());

  private static final OpCodeLookUp mcrxr_lookup = new OpCodeLookUp(X_FORM,
      "mcrxr", new mcrxr_decoder());

  private static final OpCodeLookUp lswx_lookup = new OpCodeLookUp(X_FORM,
      "lswx", new lswx_decoder());

  private static final OpCodeLookUp lwbrx_lookup = new OpCodeLookUp(X_FORM,
      "lwbrx", new lwbrx_decoder());

  private static final OpCodeLookUp lfsx_lookup = new OpCodeLookUp(X_FORM,
      "lfsx", new lfsx_decoder());

  private static final OpCodeLookUp srw_lookup = new OpCodeLookUp(X_FORM,
      "srw", new srw_decoder());

  private static final OpCodeLookUp srd_lookup = new OpCodeLookUp(X_FORM,
      "srd", new srd_decoder());

  private static final OpCodeLookUp cntlzd_lookup = new OpCodeLookUp(X_FORM,
      "cntlzd", new cntlzd_decoder());

  private static final OpCodeLookUp lfsux_lookup = new OpCodeLookUp(X_FORM,
      "lfsux", new lfsux_decoder());

  private static final OpCodeLookUp mfsr_lookup = new OpCodeLookUp(X_FORM,
      "mfsr", new mfsr_decoder());

  private static final OpCodeLookUp lswi_lookup = new OpCodeLookUp(X_FORM,
      "lswi", new lswi_decoder());

  private static final OpCodeLookUp sync_lookup = new OpCodeLookUp(X_FORM,
      "sync", new sync_decoder());

  private static final OpCodeLookUp lfdx_lookup = new OpCodeLookUp(X_FORM,
      "lfdx", new lfdx_decoder());

  private static final OpCodeLookUp lfdux_lookup = new OpCodeLookUp(X_FORM,
      "lfdux", new lfdux_decoder());

  private static final OpCodeLookUp mfsrin_lookup = new OpCodeLookUp(X_FORM,
      "mfsrin", new mfsrin_decoder());

  private static final OpCodeLookUp stswx_lookup = new OpCodeLookUp(X_FORM,
      "stswx", new stswx_decoder());

  private static final OpCodeLookUp stwbrx_lookup = new OpCodeLookUp(X_FORM,
      "stwbrx", new stwbrx_decoder());

  private static final OpCodeLookUp stfsx_lookup = new OpCodeLookUp(X_FORM,
      "stfsx", new stfsx_decoder());

  private static final OpCodeLookUp stfsux_lookup = new OpCodeLookUp(X_FORM,
      "stfsux", new stfsux_decoder());

  private static final OpCodeLookUp stswi_lookup = new OpCodeLookUp(X_FORM,
      "stswi", new stswi_decoder());

  private static final OpCodeLookUp stfdx_lookup = new OpCodeLookUp(X_FORM,
      "stfdx", new stfdx_decoder());

  private static final OpCodeLookUp dcba_lookup = new OpCodeLookUp(X_FORM,
      "dcba", new dcba_decoder());

  private static final OpCodeLookUp stfdux_lookup = new OpCodeLookUp(X_FORM,
      "stfdux", new stfdux_decoder());

  private static final OpCodeLookUp lhbrx_lookup = new OpCodeLookUp(X_FORM,
      "lhbrx", new lhbrx_decoder());

  private static final OpCodeLookUp sraw_lookup = new OpCodeLookUp(X_FORM,
      "sraw", new sraw_decoder());

  private static final OpCodeLookUp srad_lookup = new OpCodeLookUp(X_FORM,
      "srad", new srad_decoder());

  private static final OpCodeLookUp dss_lookup = new OpCodeLookUp(X_FORM,
      "dss", new dss_decoder());

  private static final OpCodeLookUp srawi_lookup = new OpCodeLookUp(X_FORM,
      "srawi", new srawi_decoder());

  private static final OpCodeLookUp sradi_lookup = new OpCodeLookUp(X_FORM,
      "sradi", new sradi_decoder());

  // private static final OpCodeLookUp sradi_lookup = new OpCodeLookUp(X_FORM,
  // "sradi", new sradi_decoder());
  private static final OpCodeLookUp eieio_lookup = new OpCodeLookUp(X_FORM,
      "eieio", new eieio_decoder());

  private static final OpCodeLookUp sthbrx_lookup = new OpCodeLookUp(X_FORM,
      "sthbrx", new sthbrx_decoder());

  private static final OpCodeLookUp extsh_lookup = new OpCodeLookUp(X_FORM,
      "extsh", new extsh_decoder());

  private static final OpCodeLookUp extsb_lookup = new OpCodeLookUp(X_FORM,
      "extsb", new extsb_decoder());

  private static final OpCodeLookUp tlbld_lookup = new OpCodeLookUp(X_FORM,
      "tlbld", new tlbld_decoder());

  private static final OpCodeLookUp icbi_lookup = new OpCodeLookUp(X_FORM,
      "icbi", new icbi_decoder());

  private static final OpCodeLookUp stfiwx_lookup = new OpCodeLookUp(X_FORM,
      "stfiwx", new stfiwx_decoder());

  private static final OpCodeLookUp extsw_lookup = new OpCodeLookUp(X_FORM,
      "extsw", new extsw_decoder());

  private static final OpCodeLookUp tlbli_lookup = new OpCodeLookUp(X_FORM,
      "tlbli", new tlbli_decoder());

  private static final OpCodeLookUp dcbz_lookup = new OpCodeLookUp(X_FORM,
      "dcbz", new dcbz_decoder());
  ...
  protected OpCodeLookUp getOpCodeLookUp(int primaryOpcode, int secondaryOpcode) {
    ...
    case 8:
      return subfc_lookup;
    case 9:
      return mulhdu_lookup;
    case 10:
      return addc_lookup;
    case 11:
      return mulhwu_lookup;
    case 19:
      return mfcr_lookup;
    case 20:
      return lwarx_lookup;
    ...
    case 235:
      return mullw_lookup;
    case 242:
      return mtsrin_lookup;
    case 246:
      return dcbtst_lookup;
    case 247:
      return stbux_lookup;
    case 266:
      return add_lookup;
    ...
    case 631:
      return lfdux_lookup;
    case 648:
      return subfeo_lookup;
    case 650:
      return addeo_lookup;
    case 659:
      return mfsrin_lookup;
    case 661:
      return stswx_lookup;
    case 662:
      return stwbrx_lookup;
    case 663:
      return stfsx_lookup;
    case 695:
      return stfsux_lookup;
    case 712:
      return subfzeo_lookup;
    case 714:
      return addzeo_lookup;
    case 725:
      return stswi_lookup;
    case 727:
      return stfdx_lookup;
    case 744:
      return subfmeo_lookup;
    case 746:
      return addmeo_lookup;
    case 747:
      return mullwo_lookup;
    case 758:
      return dcba_lookup;
    case 759:
      return stfdux_lookup;
    case 772:
      return nego_lookup;
    case 778:
      return addo_lookup;
    ...
    case 854:
      return eieio_lookup;
    case 918:
      return sthbrx_lookup;
    case 922:
      return extsh_lookup;
    case 954:
      return extsb_lookup;
    case 971:
      return divwuo_lookup;
    case 978:
      return tlbld_lookup;
    case 982:
      return icbi_lookup;
    case 983:
      return stfiwx_lookup;
    case 986:
      return extsw_lookup;
    case 1003:
      return divwo_lookup;
...
final class extended59_decoder extends PPC_InstructionDecoder {
  private static final OpCodeLookUp fdivs_lookup = new OpCodeLookUp(A_FORM,
      "fdivs", new fdivs_decoder());

  private static final OpCodeLookUp fsubs_lookup = new OpCodeLookUp(A_FORM,
      "fsubs", new fsubs_decoder());

  private static final OpCodeLookUp fadds_lookup = new OpCodeLookUp(A_FORM,
      "fadds", new fadds_decoder());

  private static final OpCodeLookUp fsqrts_lookup = new OpCodeLookUp(A_FORM,
      "fsqrts", new fsqrts_decoder());

  private static final OpCodeLookUp fres_lookup = new OpCodeLookUp(A_FORM,
      "fres", new fres_decoder());

  private static final OpCodeLookUp fmuls_lookup = new OpCodeLookUp(A_FORM,
      "fmuls", new fmuls_decoder());

  private static final OpCodeLookUp fmsubs_lookup = new OpCodeLookUp(A_FORM,
      "fmsubs", new fmsubs_decoder());

  private static final OpCodeLookUp fmadds_lookup = new OpCodeLookUp(A_FORM,
      "fmadds", new fmadds_decoder());

  private static final OpCodeLookUp fnmsubs_lookup = new OpCodeLookUp(A_FORM,
      "fnmsubs", new fnmsubs_decoder());

  private static final OpCodeLookUp fnmadds_lookup = new OpCodeLookUp(A_FORM,
      "fnmadds", new fnmadds_decoder());
  ...
  protected OpCodeLookUp getOpCodeLookUp(int primaryOpcode, int secondaryOpcode) {
    ...
    case 25:
      return fmuls_lookup;
    case 28:
      return fmsubs_lookup;
    case 29:
      return fmadds_lookup;
    case 30:
      return fnmsubs_lookup;
    case 31:
      return fnmadds_lookup;
    default:
      break;
    }
    return null;
...
final class extended63_decoder extends PPC_InstructionDecoder {
  // The decoders for the X_Form instructions
  private static final OpCodeLookUp fcmpu_lookup = new OpCodeLookUp(X_FORM,
      "fcmpu", new fcmpu_decoder());

  private static final OpCodeLookUp frsp_lookup = new OpCodeLookUp(X_FORM,
      "frsp", new frsp_decoder());

  private static final OpCodeLookUp fctiw_lookup = new OpCodeLookUp(X_FORM,
      "fctiw", new fctiw_decoder());

  private static final OpCodeLookUp fctiwz_lookup = new OpCodeLookUp(X_FORM,
      "fctiwz", new fctiwz_decoder());

  private static final OpCodeLookUp fcmpo_lookup = new OpCodeLookUp(X_FORM,
      "fcmpo", new fcmpo_decoder());

  private static final OpCodeLookUp mtfsb1_lookup = new OpCodeLookUp(X_FORM,
      "mtfsb1", new mtfsb1_decoder());

  private static final OpCodeLookUp fneg_lookup = new OpCodeLookUp(X_FORM,
      "fneg", new fneg_decoder());

  private static final OpCodeLookUp mcrfs_lookup = new OpCodeLookUp(X_FORM,
      "mcrfs", new mcrfs_decoder());

  private static final OpCodeLookUp mtfsb0_lookup = new OpCodeLookUp(X_FORM,
      "mtfsb0", new mtfsb0_decoder());

  private static final OpCodeLookUp fmr_lookup = new OpCodeLookUp(X_FORM,
      "fmr", new fmr_decoder());

  private static final OpCodeLookUp mtfsfi_lookup = new OpCodeLookUp(X_FORM,
      "mtfsfi", new mtfsfi_decoder());

  private static final OpCodeLookUp fnabs_lookup = new OpCodeLookUp(X_FORM,
      "fnabs", new fnabs_decoder());

  private static final OpCodeLookUp fabs_lookup = new OpCodeLookUp(X_FORM,
      "fabs", new fabs_decoder());

  private static final OpCodeLookUp mffs_lookup = new OpCodeLookUp(X_FORM,
      "mffs", new mffs_decoder());

  private static final OpCodeLookUp fctid_lookup = new OpCodeLookUp(X_FORM,
      "fctid", new fctid_decoder());

  private static final OpCodeLookUp fctidz_lookup = new OpCodeLookUp(X_FORM,
      "fctidz", new fctidz_decoder());

  private static final OpCodeLookUp fcfid_lookup = new OpCodeLookUp(X_FORM,
      "fcfid", new fcfid_decoder());

  // The decoders for the XFL_Form instructions
  private static final OpCodeLookUp mtfsf_lookup = new OpCodeLookUp(XFL_FORM,
      "mtfsf", new mtfsf_decoder());

  // The decoders for the A_Form instructions
  private static final OpCodeLookUp fdiv_lookup = new OpCodeLookUp(A_FORM,
      "fdiv", new fdiv_decoder());

  private static final OpCodeLookUp fsub_lookup = new OpCodeLookUp(A_FORM,
      "fsub", new fsub_decoder());

  private static final OpCodeLookUp fadd_lookup = new OpCodeLookUp(A_FORM,
      "fadd", new fadd_decoder());

  private static final OpCodeLookUp fmul_lookup = new OpCodeLookUp(A_FORM,
      "fmul", new fmul_decoder());

  private static final OpCodeLookUp fmsub_lookup = new OpCodeLookUp(A_FORM,
      "fmsub", new fmsub_decoder());

  private static final OpCodeLookUp fmadd_lookup = new OpCodeLookUp(A_FORM,
      "fmadd", new fmadd_decoder());

  private static final OpCodeLookUp fnmsub_lookup = new OpCodeLookUp(A_FORM,
      "fnmsub", new fnmsub_decoder());

  private static final OpCodeLookUp fnmadd_lookup = new OpCodeLookUp(A_FORM,
      "fnmadd", new fnmadd_decoder());
  ...
  protected OpCodeLookUp getOpCodeLookUp(int primaryOpcode, int secondaryOpcode) {
    ...
    case 21:
      return fadd_lookup;
    case 22:
      throw new Error("fsqrt!");
    case 32:
      return fcmpo_lookup;
    case 38:
      return mtfsb1_lookup;
    case 40:
      return fneg_lookup;
    case 64:
      return mcrfs_lookup;
    case 70:
      return mtfsb0_lookup;
    case 72:
      return fmr_lookup;
    case 134:
      return mtfsfi_lookup;
    case 136:
      return fnabs_lookup;
    case 264:
      return fabs_lookup;
    ...
    case 29:
      return fmadd_lookup;
    case 30:
      return fnmsub_lookup;
    case 31:
      return fnmadd_lookup;
    default:
      break;
    }
    return null;
...
final class tdi_decoder extends PPC_InstructionDecoder {
   ...
   *          the process space of the interpretation
   * @param pc
   *          the address of the instruction to interpret
   * @return the next instruction interpreter
   */
  public PPC_InstructionDecoder interpretInstruction(PPC_ProcessSpace ps)
      throws BadInstructionException {
...
final class mulli_decoder extends PPC_InstructionDecoder {
  ...
  /**
   * Translate mulli (multiply low immediate): prod[0-48] <- (rA) * SIMM (rD) <-
   * prod[16-48]
   */
  protected int translateD_FORM(PPC2IR ppc2ir, PPC_Laziness lazy, int pc,
    ...
    ppc2ir.appendInstruction(Binary.create(INT_MUL, ppc2ir
        ...
        new IntConstantOperand(EXTS(simm, 16))));
    return pc + 4;
  }
}
...
final class subfic_decoder extends PPC_InstructionDecoder {
  ...
  /**
   * Translate subfic (subtract from immediate carrying): <listing> rD <- ¬ (rA) +
   * EXTS(SIMM) + 1 </listing> Also modifies XER[CA]. Equivalent to: <listing>
   * rD <- EXTS(SIMM) - (rA) </listing>
   */
  protected int translateD_FORM(PPC2IR ppc2ir, PPC_Laziness lazy, int pc,
      int inst, int opcode, int rD, int rA, int imm) {
    // Get rA & simm
    RegisterOperand reg_rA = ppc2ir.getGPRegister(rA);
    int simm = EXTS(imm, 16);

    // Perform subtract for rD
    ppc2ir.appendInstruction(Binary.create(INT_SUB, ppc2ir
        .getGPRegister(rD), new IntConstantOperand(simm), reg_rA.copyRO()));
    // Set XER CA based on result
    ppc2ir.appendInstruction(BooleanCmp.create(BOOLEAN_CMP_INT,
        ppc2ir.getXER_CA_Register(), new IntConstantOperand(simm), reg_rA
            .copyRO(), ConditionOperand.LOWER(), BranchProfileOperand
            .unlikely()));
    return pc + 4;
  }
}
...
final class cmpli_decoder extends PPC_InstructionDecoder {
  ...
  /**
   * Translate cmpli (compare logical immediate): <listing> a <- (rA) if a <U
   * ((16)0 || UIMM) then c <- 0b100 else if a >U ((16)0 || UIMM) then c <-
   * 0b010 else c <- 0b001 CR[(4 * crfD)-(4 * crfD + 3)] <- c || XER[SO]
   * </listing>
   */
  protected int translateD_FORM(PPC2IR ppc2ir, PPC_Laziness lazy, int pc,
      int inst, int opcode, int crfD0L, int rA, int uimm) {
    int crfD = crfD0L >>> 2;

    setCRfield(ppc2ir, lazy, crfD, ppc2ir.getGPRegister(rA),
        new IntConstantOperand(uimm), UNSIGNED_INT_CMP);
...
final class cmpi_decoder extends PPC_InstructionDecoder {
  ...
  /**
   * Translate cmpi (compare immediate): <listing> a <- (rA) if a < EXTS(SIMM)
   * then c <- 0b100 else if a > EXTS(SIMM) then c <- 0b010 else c <- 0b001
   * CR[(4 * crfD)-(4 * crfD + 3)] <- c || XER[SO] </listing>
   */
  protected int translateD_FORM(PPC2IR ppc2ir, PPC_Laziness lazy, int pc,
      int inst, int opcode, int crfD0L, int rA, int imm) {
    int crfD = crfD0L >>> 2;

    setCRfield(ppc2ir, lazy, crfD, ppc2ir.getGPRegister(rA),
        new IntConstantOperand(EXTS(imm, 16)), SIGNED_INT_CMP);
...
final class addic_decoder extends PPC_InstructionDecoder {
  ...
  /**
   * Translate addic (add immediate carrying) <listing> rD <- (rA) + EXTS(SIMM)
   * </listing> Other simplified mnemonics: subic
   */
  protected int translateD_FORM(PPC2IR ppc2ir, PPC_Laziness lazy, int pc,
      int inst, int opcode, int rD, int rA, int imm) {
    // Get rA & simm
    RegisterOperand reg_rA = ppc2ir.getGPRegister(rA);
    RegisterOperand reg_rD = ppc2ir.getGPRegister(rD);
    int simm = EXTS(imm, 16);

    // Perform add for rD
    ppc2ir.appendInstruction(Binary.create(INT_ADD, reg_rD,
        new IntConstantOperand(simm), reg_rA.copyRO()));
    // Set XER CA based on result
    ppc2ir.appendInstruction(BooleanCmp.create(BOOLEAN_CMP_INT,
        ppc2ir.getXER_CA_Register(), reg_rD.copyRO(),
        new IntConstantOperand(simm), // could equally be reg_rA
        ConditionOperand.LOWER(), BranchProfileOperand.unlikely()));
    return pc + 4;
  }
}
...
final class addic__decoder extends PPC_InstructionDecoder {
  ...
  /**
   * Translate addic. (add immediate carrying and record) <listing> rD <- (rA) +
   * EXTS(SIMM) </listing> Other simplified mnemonics: subfic.
   */
  protected int translateD_FORM(PPC2IR ppc2ir, PPC_Laziness lazy, int pc,
      int inst, int opcode, int rD, int rA, int imm) {
    // Get rA & simm
    ...
    setCRfield(ppc2ir, lazy, 0, ppc2ir.getGPRegister(rD),
        new IntConstantOperand(0), SIGNED_INT_CMP);
...
final class addi_decoder extends PPC_InstructionDecoder {
  ...
  /**
   * Translate addi (add immediate) <listing> if rA = 0 then rD <- EXTS(SIMM)
   * else rD <- (rA) + EXTS(SIMM) </listing> Other simplified mnemonics: li, la,
   * subi
   */
  protected int translateD_FORM(PPC2IR ppc2ir, PPC_Laziness lazy, int pc,
      int inst, int opcode, int rD, int rA, int imm) {
    ...
    if (rA == 0) {
      ppc2ir.appendInstruction(Move.create(INT_MOVE, ppc2ir
          .getGPRegister(rD), new IntConstantOperand(simm)));
    } else {
      ppc2ir.appendInstruction(Binary.create(INT_ADD, ppc2ir
          .getGPRegister(rD), ppc2ir.getGPRegister(rA),
          new IntConstantOperand(simm)));
    }
    return pc + 4;
  }
}
...
final class addis_decoder extends PPC_InstructionDecoder {
  ...
  /**
   * Translate addis (add immediate shifted) <listing> if rA = 0 then rD <-
   * EXTS(SIMM || (16)0) else rD <- (rA) + EXTS(SIMM || (16)0) </listing> Other
   * simplified mnemonics: lis, subis
   */
  protected int translateD_FORM(PPC2IR ppc2ir, PPC_Laziness lazy, int pc,
      int inst, int opcode, int rD, int rA, int imm) {
    int simm = EXTS(imm, 16);
    ...
    if (rA == 0) {
      ppc2ir.appendInstruction(Move.create(INT_MOVE, ppc2ir
          .getGPRegister(rD), new IntConstantOperand(simm << 16)));
    } else {
      ppc2ir.appendInstruction(Binary.create(INT_ADD, ppc2ir
          .getGPRegister(rD), ppc2ir.getGPRegister(rA),
          new IntConstantOperand(simm << 16)));
    }
    return pc + 4;
  }
}
...
final class ori_decoder extends PPC_InstructionDecoder {
  ...
  /**
   * Translate ori (or immediate) <listing> rA <- (rS) | ((16)0 || UIMM)
   * </listing> Other simplified mnemonics: nop
   */
  protected int translateD_FORM(PPC2IR ppc2ir, PPC_Laziness lazy, int pc,
...
final class oris_decoder extends PPC_InstructionDecoder {
  ...
  /**
   * Translate oris (or immediate shifted) <listing> rA <- (rS) | (UIMM ||
   * (16)0) </listing>
   */
  protected int translateD_FORM(PPC2IR ppc2ir, PPC_Laziness lazy, int pc,
      int inst, int opcode, int rS, int rA, int uimm) {
    if ((uimm == 0) && (rA == rS)) {
      ...
      ppc2ir.appendInstruction(Binary.create(INT_OR, ppc2ir
          .getGPRegister(rA), ppc2ir.getGPRegister(rS),
          new IntConstantOperand(uimm << 16)));
    }
    return pc + 4;
  }
}
...
final class xori_decoder extends PPC_InstructionDecoder {
  ...
  /**
   * Translate xori (xor immediate) <listing> rA <- (rS) (+) ((16)0 || UIMM)
   * </listing>
   */
  protected int translateD_FORM(PPC2IR ppc2ir, PPC_Laziness lazy, int pc,
      int inst, int opcode, int rS, int rA, int uimm) {
...
final class xoris_decoder extends PPC_InstructionDecoder {
  ...
  /**
   * Translate xoris (xor immediate shifted) <listing> rA <- (rS) (+) (UIMM ||
   * (16)0) </listing>
   */
  protected int translateD_FORM(PPC2IR ppc2ir, PPC_Laziness lazy, int pc,
      int inst, int opcode, int rS, int rA, int uimm) {
...
final class andi__decoder extends PPC_InstructionDecoder {
  ...
  /**
   * Translate andi. (and immediate (and record)) <listing> rA <- (rS) & ((16)0 ||
   * UIMM) </listing>
   */
  protected int translateD_FORM(PPC2IR ppc2ir, PPC_Laziness lazy, int pc,
      int inst, int opcode, int rS, int rA, int uimm) {
    RegisterOperand result = ppc2ir.getGPRegister(rA);
    ppc2ir.appendInstruction(Binary.create(INT_AND, result,
        ppc2ir.getGPRegister(rS), new IntConstantOperand(uimm)));
    // Make copies of operands for lazy state
...
final class andis__decoder extends PPC_InstructionDecoder {
  ...
  /**
   * Translate andis. (and immediate shifted (and record)) <listing> rA <- (rS) &
   * (UIMM || (16)0) </listing>
   */
  protected int translateD_FORM(PPC2IR ppc2ir, PPC_Laziness lazy, int pc,
      int inst, int opcode, int rS, int rA, int uimm) {
    RegisterOperand result = ppc2ir.getGPRegister(rA);
    ppc2ir.appendInstruction(Binary.create(INT_AND, result,
        ppc2ir.getGPRegister(rS), new IntConstantOperand(uimm << 16)));
    // Make copies of operands for lazy state
    setCRfield(ppc2ir, lazy, 0, ppc2ir.getGPRegister(rA),
        new IntConstantOperand(0), SIGNED_INT_CMP);
    return pc + 4;
...
final class lwz_decoder extends PPC_InstructionDecoder {
  ...
  /**
   * Translate lwz (load word and zero) <listing> if rA = 0 then b <- 0 else b <-
   * (rA) EA <- b+ EXTS(d) rD <- MEM(EA, 4) </listing>
   */
  protected int translateD_FORM(PPC2IR ppc2ir, PPC_Laziness lazy, int pc,
      int inst, int opcode, int rD, int rA, int d) {
    RegisterOperand EA = ppc2ir.getTempInt(0);
    d = EXTS(d, 16);
    if (rA == 0) {
      // EA = EXTS(d)
      ppc2ir.appendInstruction(Move.create(INT_MOVE, EA,
          new IntConstantOperand(d)));
    } else {
      // EA = rA + EXTS(d)
      ppc2ir.appendInstruction(Binary.create(INT_ADD, EA, ppc2ir
          .getGPRegister(rA), new IntConstantOperand(d)));
    }
...
final class lwzu_decoder extends PPC_InstructionDecoder {
  ...
  /**
   * Translate lwzu (load word and zero with update) <listing> EA <- rA +
   * EXTS(d) rD <- MEM(EA, 4) rA <- EA </listing>
   */
  protected int translateD_FORM(PPC2IR ppc2ir, PPC_Laziness lazy, int pc,
      int inst, int opcode, int rD, int rA, int d) {
    RegisterOperand EA = ppc2ir.getTempInt(0);
...
final class lbz_decoder extends PPC_InstructionDecoder {
  ...
  /**
   * Translate lbz (load byte and zero) <listing> if rA = 0 then b <- 0 else b <-
   * (rA) EA <- b+ EXTS(d) rD <- (24)0 || MEM(EA, 1) </listing>
   */
  protected int translateD_FORM(PPC2IR ppc2ir, PPC_Laziness lazy, int pc,
      int inst, int opcode, int rD, int rA, int d) {
    RegisterOperand EA = ppc2ir.getTempInt(0);
    ...
    // Load value from memory at EA into rD.
    RegisterOperand result = ppc2ir.getGPRegister(rD);
...
/**
 * The decoder for the lbzu instruction <listing> EA <- rA + EXTS(d) rD <- (24)0 ||
 * MEM(EA, 1) rA <- EA </listing>
 */
...
/**
 * The decoder for the stw instruction <listing> if rA = 0 then b <- 0 else b <-
 * (rA) EA <- b+ EXTS(d) MEM(EA, 4) <- rS </listing>
 */
final class stw_decoder extends PPC_InstructionDecoder {
   ...
   */
  protected int translateD_FORM(PPC2IR ppc2ir, PPC_Laziness lazy, int pc,
      int inst, int opcode, int rS, int rA, int d) {
    RegisterOperand EA = ppc2ir.getTempInt(0);
    d = EXTS(d, 16);
    if (rA == 0) {
      // EA = EXTS(d)
...
final class stwu_decoder extends PPC_InstructionDecoder {
  ...
  /**
   * Translate stwu (store word with update) <listing> EA <- rA + EXTS(d)
   * MEM(EA, 4) <- rS rA <- EA </listing>
   */
  protected int translateD_FORM(PPC2IR ppc2ir, PPC_Laziness lazy, int pc,
      int inst, int opcode, int rS, int rA, int d) {
    RegisterOperand EA = ppc2ir.getTempInt(0);
...
/**
 * The decoder for the stb (store byte) instruction <listing> if rA = 0 then b <-
 * 0 else b <- (rA) EA <- b+ EXTS(d) MEM(EA, 1) <- rS[24-31] </listing>
 */
...
/**
 * The decoder for the stbu (store byte with update) instruction <listing> EA <-
 * rA + EXTS(d) MEM(EA, 1) <- rS[24-31] rA <- EA </listing>
 */
...
/**
 * The decoder for the lhz (load half word and zero) instruction <listing> if rA =
 * 0 then b <- 0 else b <- (rA) EA <- b+ EXTS(d) rD <- (16)0 || MEM(EA, 2)
 * </listing>
 */
 ...
 * <listing> EA <- (rA) + EXTS(d) rD <- (16)0 || MEM(EA, 2) rA <- EA </listing>
 */
...
/**
 * The decoder for the lha (load half word algabraeic) instruction <listing> if
 * rA = 0 then b <- 0 else b <- (rA) EA <- b+ EXTS(d) rD <- EXTS(MEM(EA, 2))
 * </listing>
 */
 ...
 * <listing> EA <- (rA)+ EXTS(d) rD <- EXTS(MEM(EA, 2)) rA <- EA </listing>
 */
...
/**
 * The decoder for the sth (store half word) instruction <listing> if rA = 0
 * then b <- 0 else b <- (rA) EA <- b+ EXTS(d) MEM(EA, 2) <- rS[16-31]
 * </listing>
 */
...
/**
 * The decoder for the sthu (store half word with update) instruction <listing>
 * EA <- (rA) + EXTS(d) MEM(EA, 2) <- rS[16-31] rA <- EA </listing>
 */
...
/**
 * The decoder for the lfs (load floating-point single) instruction <listing> if
 * rA = 0 then b <- 0 else b <- (rA) EA <- b+ EXTS(d) frD <- DOUBLE(MEM(EA, 4))
 * </listing>
 */
final class lfs_decoder extends PPC_InstructionDecoder {
   ...
   */
  protected int translateD_FORM(PPC2IR ppc2ir, PPC_Laziness lazy, int pc,
      int inst, int opcode, int frD, int rA, int d) {
    RegisterOperand EA = ppc2ir.getTempInt(0);
    d = EXTS(d, 16);
    if (rA == 0) {
      // EA = EXTS(d)
    ...
    RegisterOperand tempInt = ppc2ir.getTempInt(0 + 1);
    ppc2ir.ps.memory.translateLoad32(EA.copyRO(), tempInt);
    // Convert bit pattern to a float.
    RegisterOperand tempFloat = ppc2ir.getTempFloat(0);
 ...
 * <listing> EA <- (rA)+ EXTS(d) frD <- DOUBLE(MEM(EA, 4)) rA <- EA </listing>
 */
...
/**
 * The decoder for the lfd (load floating-point double) instruction <listing> if
 * rA = 0 then b <- 0 else b <- (rA) EA <- b+ EXTS(d) frD <- MEM(EA, 8)
 * </listing>
 */
final class lfd_decoder extends PPC_InstructionDecoder {
  ...
  public PPC_InstructionDecoder interpretInstruction(PPC_ProcessSpace ps)
    ...
    int d = EXTS(bits(ps.currentInstruction, 16, 31), 16);
    int EA = (rA == 0) ? d : (ps.getRegister(rA) + d);
    long value = (((long) ps.memory.load32(EA)) << 32)
        | (((long) ps.memory.load32(EA + 4)) & 0xFFFFFFFF);
  ...
  protected int translateD_FORM(PPC2IR ppc2ir, PPC_Laziness lazy, int pc,
      int inst, int opcode, int frD, int rA, int d) {
    RegisterOperand EA = ppc2ir.getTempInt(0);
    RegisterOperand EA2 = ppc2ir.getTempInt(0 + 1);
    d = EXTS(d, 16);
    if (rA == 0) {
      // EA = EXTS(d)
      ppc2ir.appendInstruction(Move.create(INT_MOVE, EA,
      ...
      ppc2ir.appendInstruction(Move.create(INT_MOVE, EA2,
          new IntConstantOperand(d + 4)));
    } else {
      // EA = rA + EXTS(d)
      ppc2ir.appendInstruction(Binary.create(INT_ADD, EA, ppc2ir
          .getGPRegister(rA), new IntConstantOperand(d)));
      ppc2ir.appendInstruction(Binary.create(INT_ADD, EA2, ppc2ir
          .getGPRegister(rA), new IntConstantOperand(d + 4)));
    }
    // Load value from memory at EA & EA2 into tempInt & tempInt2 (msb at low
    // address)
    RegisterOperand tempInt = ppc2ir.getTempInt(0 + 2);
    RegisterOperand tempInt2 = ppc2ir.getTempInt(0 + 3);
    ppc2ir.ps.memory.translateLoad32(EA.copyRO(), tempInt);
    ppc2ir.ps.memory.translateLoad32(EA2.copyRO(), tempInt2);

    // Merge ints into a long
    // tempLong = ((long)tempInt << 32) | (long)tempInt2
    RegisterOperand tempLong = ppc2ir.getTempLong(0);
    RegisterOperand tempLong2 = ppc2ir.getTempLong(1);
    ...
    ppc2ir.appendInstruction(Binary.create(LONG_SHL, tempLong
        .copyRO(), tempLong.copyRO(), new IntConstantOperand(32)));
    ppc2ir.appendInstruction(Binary
        .create(LONG_AND, tempLong2.copyRO(), tempLong2.copyRO(),
            new LongConstantOperand(0xffffffffl)));
 ...
 * <listing> EA <- (rA)+ EXTS(d) frD <- MEM(EA, 8) rA <- EA </listing>
 */
...
/**
 * The decoder for the stfs (store floating point single) instruction <listing>
 * if rA = 0 then b <- 0 else b <- (rA) EA <- b+ EXTS(d) MEM(EA, 4) <-
 * SINGLE(frS) </listing>
 */
final class stfs_decoder extends PPC_InstructionDecoder {
   ...
   */
  protected int translateD_FORM(PPC2IR ppc2ir, PPC_Laziness lazy, int pc,
      int inst, int opcode, int frS, int rA, int d) {
    RegisterOperand EA = ppc2ir.getTempInt(0);
    d = EXTS(d, 16);
    if (rA == 0) {
      // EA = EXTS(d)
...
/**
 * The decoder for the stfsu (store floating point single with update)
 * instruction <listing> EA <- (rA)+ EXTS(d) MEM(EA, 4) <- SINGLE(frS) rA <- EA
 * </listing>
 */
...
/**
 * The decoder for the stfd (store floating-point double) instruction <listing>
...
final class stfd_decoder extends PPC_InstructionDecoder {
  ...
  protected int translateD_FORM(PPC2IR ppc2ir, PPC_Laziness lazy, int pc,
    ...
    ppc2ir.appendInstruction(Binary.create(LONG_USHR, tempLong2,
        tempLong.copyRO(), new IntConstantOperand(32)));
    ppc2ir
        .appendInstruction(Binary.create(LONG_AND, tempLong
            .copyRO(), tempLong.copyRO(), new LongConstantOperand(
            0xffffffffl)));
...
/**
 * The decoder for the stfdu (store floating-point double with update)
 * instruction <listing> EA <- (rA)+ EXTS(d) MEM(EA, 8) <- (frS) rA <- EA
 * </listing>
 */
...
final class mcrf_decoder extends PPC_InstructionDecoder {
  ...
  /**
   * Translate mcrf (move condition register field) <listing> CR[(4 * crfD)
   * through (4 * crfD + 3)] <- CR[(4 * crfS) through (4 * crfS + 3)] </listing>
   */
  protected int translateXL_FORM(PPC2IR ppc2ir, PPC_Laziness lazy, int pc,
    ...
    if (crfD != crfS) {
      RegisterOperand ltS = ppc2ir.getCR_Lt_Register(crfS);
      RegisterOperand gtS = ppc2ir.getCR_Gt_Register(crfS);
      RegisterOperand eqS = ppc2ir.getCR_Eq_Register(crfS);
      RegisterOperand soS = ppc2ir.getCR_SO_Register(crfS);

      RegisterOperand ltD = ppc2ir.getCR_Lt_Register(crfD);
      RegisterOperand gtD = ppc2ir.getCR_Gt_Register(crfD);
      RegisterOperand eqD = ppc2ir.getCR_Eq_Register(crfD);
      RegisterOperand soD = ppc2ir.getCR_SO_Register(crfD);
...
final class bclr_decoder extends PPC_InstructionDecoder {
  ...
  /**
   * Translate bclr (branch conditional to link register) <listing> if ¬ BO[2]
   * then CTR <- CTR - 1 ctr_ok <- BO[2] | ((CTR != 0) (+) BO[3]) cond_ok <-
   * BO[0] | (CR[BI] == BO[1]) if ctr_ok & cond_ok then NIA <-iea LR || 0b00 if
   * LK then LR <-iea CIA + 4 </listing> Other simplified mnemonics: bltlr,
   * bnelr, bdnzlr
   */
  protected int translateXL_FORM(PPC2IR ppc2ir, PPC_Laziness lazy, int pc,
    ...
    // 3: condition field test and branch to instructionEndBlock if true (or
    // false)
    // plant branch block (and if lk alter lr value)

    // The block gone to if the branch isn't taken
    BasicBlock instructionEndBlock = ppc2ir.getNextBlock();
    ...
    if ((BO & 4) == 0) {
      ...
      plantBranchToBlockDependentOnCTR(ppc2ir, instructionEndBlock,
          branch_if_ctr_zero, likely_to_fallthrough);
    }
    if ((BO & 16) == 0) {
      boolean branch_if_cond_true = ((BO & 8) != 0);
    ...
    // Plant branch block
    RegisterOperand branchAddress;
    if (lk != 0) {
      // @todo: record the pc as an address that set lr
      RegisterOperand lr = ppc2ir.getLRRegister();
...
final class crnor_decoder extends PPC_InstructionDecoder {
  ...
  /**
   * Translate crnor (condition register nor) <listing> CR[crbD] <- ¬(CR[crbA] |
   * CR[crbB]) </listing> Other simplified mnemonics: crnot
   */
  protected int translateXL_FORM(PPC2IR ppc2ir, PPC_Laziness lazy, int pc,
    ...
    RegisterOperand cr = ppc2ir.getCRRegister();
    if ((crbA == crbB) && (crbA == crbD)) { // crnot crbA, crbA -> just twiddle
      ...
      ppc2ir.appendInstruction(Binary.create(INT_AND, tempInt3
          .copyRO(), tempInt3.copyRO(), new IntConstantOperand(
          0x80000000 >>> crbD)));
      // Clear desitination bit
      ppc2ir.appendInstruction(Binary.create(INT_AND,
          cr.copyRO(), cr.copyRO(), new IntConstantOperand(
              ~(0x80000000 >>> crbD))));
      // Combine result
...
final class crxor_decoder extends PPC_InstructionDecoder {
  ...
  /**
   * Translate crxor (condition register xor) <listing> CR[crbD] <- CR[crbA] (+)
   * CR[crbB] </listing> Other simplified mnemonics: crclr
   */
  protected int translateXL_FORM(PPC2IR ppc2ir, PPC_Laziness lazy, int pc,
      int inst, int opcode, int crbD, int crbA, int crbB, int secondaryOpcode,
      int zero) {
    ...
    if ((crbA == crbB) && (crbA == crbD)) { // crclr crbD -> just zero bit in
                                            // position
      RegisterOperand crb_regOp = ppc2ir.getCRB_Register(crbA);
      ppc2ir.appendInstruction(Move.create(INT_MOVE, crb_regOp,
          new IntConstantOperand(0)));
    } else {
      RegisterOperand crbA_regOp = ppc2ir.getCRB_Register(crbA);
      RegisterOperand crbB_regOp = ppc2ir.getCRB_Register(crbB);
      RegisterOperand crbD_regOp = ppc2ir.getCRB_Register(crbD);
      ppc2ir.appendInstruction(BooleanCmp.create(BOOLEAN_CMP_INT,
          crbD_regOp, crbB_regOp, crbA_regOp, ConditionOperand.NOT_EQUAL(),
          BranchProfileOperand.unlikely()));
    }
    return pc + 4;
  }
...
final class creqv_decoder extends PPC_InstructionDecoder {
  ...
  /**
   * Translate creqv (condition register equivalent) <listing> CR[crbD] <-
   * CR[crbA] == CR[crbB] </listing> Other simplified mnemonics: crset
   */
  protected int translateXL_FORM(PPC2IR ppc2ir, PPC_Laziness lazy, int pc,
      int inst, int opcode, int crbD, int crbA, int crbB, int secondaryOpcode,
      int zero) {
    ...
    if ((crbA == crbB) && (crbA == crbD)) { // crset crbD -> just set bit in
      ...
      int bitMask = 0x80000000 >>> crbA;
      ppc2ir.appendInstruction(Binary.create(INT_OR, cr.copyRO(),
          cr, new IntConstantOperand(bitMask)));
    } else {
      ppc2ir.resolveLazinessCrBit(lazy, crbB);
      ppc2ir.resolveLazinessCrBit(lazy, crbD);
...
final class cror_traslator extends PPC_InstructionDecoder {
  ...
  /**
   * Translate cror (condition register or) <listing> CR[crbD] <- CR[crbA] |
   * CR[crbB] </listing> Other simplified mnemonics: crclr
   */
  protected int translateXL_FORM(PPC2IR ppc2ir, PPC_Laziness lazy, int pc,
      int inst, int opcode, int crbD, int crbA, int crbB, int secondaryOpcode,
    ...
    if ((crbA == crbB) && (crbA == crbD)) {
      ...
      ppc2ir.appendInstruction(Binary.create(INT_AND, tempInt1
          .copyRO(), tempInt1.copyRO(), new IntConstantOperand(
          0x80000000 >>> crbD)));
      // Clear desitination bit
      ppc2ir.appendInstruction(Binary.create(INT_AND,
          cr.copyRO(), cr.copyRO(), new IntConstantOperand(
...
final class bcctr_decoder extends PPC_InstructionDecoder {
  ...
  /**
   * Translate bcctr (branch conditional to count register) <listing> if ¬ BO[2]
   * then CTR <- CTR - 1 ctr_ok <- BO[2] | ((CTR != 0) (+) BO[3]) cond_ok <-
   * BO[0] | (CR[BI] == BO[1]) if ctr_ok & cond_ok then NIA <-iea CTR || 0b00 if
   * LK then LR <-iea CIA + 4 </listing> Other simplified mnemonics: bltctr,
   * bnectr
   */
  protected int translateXL_FORM(PPC2IR ppc2ir, PPC_Laziness lazy, int pc,
      int inst, int opcode, int BO, int BI, int zero, int secondaryOpcode,
    ...
    RegisterOperand branchAddress = ppc2ir.getCTRRegister();
...
final class subfc_decoder extends PPC_InstructionDecoder {
  ...
  /**
   * Translate subfc (subtract from carrying): <listing> rD <- ¬ (rA) + (rB) + 1
   * </listing> Also modifies XER[CA]. Equivalent to: <listing> rD <- (rB) -
   * (rA) </listing>
   */
  protected int translateXO_FORM(PPC2IR ppc2ir, PPC_Laziness lazy, int pc,
    ...
    // Get rA & rB
    RegisterOperand reg_rA = ppc2ir.getGPRegister(rA);
    RegisterOperand reg_rB = ppc2ir.getGPRegister(rB);
...
final class addc_decoder extends PPC_InstructionDecoder {
  ...
  /**
   * Translate addc (add carrying): <listing> rD <- (rA) + (rB) </listing> Also
   * modifies XER[CA].
   */
  protected int translateXO_FORM(PPC2IR ppc2ir, PPC_Laziness lazy, int pc,
      int inst, int opcode, int rD, int rA, int rB, int OE,
    ...
    if (Rc != 0) {
      // Set condition codes
      setCRfield(ppc2ir, lazy, 0, reg_rD.copyRO(),
          new IntConstantOperand(0), SIGNED_INT_CMP);
    }
...
final class mulhwu_decoder extends PPC_InstructionDecoder {
  ...
  /**
   * Translate mulhwu (multiply high word unsigned): <listing> prod[0-63] <- rA *
   * rB rD <- prod[0-31] </listing>
   */
  protected int translateXO_FORM(PPC2IR ppc2ir, PPC_Laziness lazy, int pc,
      int inst, int opcode, int rD, int rA, int rB, int OE,
      int secondaryOpcode, int Rc) {
    // Get rA & rB as longs
    RegisterOperand long_rA = ppc2ir.getTempLong(0);
    RegisterOperand long_rB = ppc2ir.getTempLong(1);
    RegisterOperand long_rD = ppc2ir.getTempLong(2);
    ...
    ppc2ir.appendInstruction(Binary.create(LONG_AND, long_rA
        .copyRO(), long_rA.copyRO(), new LongConstantOperand(0xffffffffL)));
    ppc2ir.appendInstruction(Binary.create(LONG_AND, long_rB
        .copyRO(), long_rB.copyRO(), new LongConstantOperand(0xffffffffL)));
    // Perform multiply
    ...
    ppc2ir.appendInstruction(Binary.create(LONG_USHR, long_rD
        .copyRO(), long_rD.copyRO(), new IntConstantOperand(32)));
    // Set rD
...
final class subf_decoder extends PPC_InstructionDecoder {
  ...
  /**
   * Translate subf (subtract from): <listing> rD <- ¬ (rA) + (rB) + 1
   * </listing> Equivalent to: <listing> rD <- (rB) - (rA) </listing> Other
   * simplified mnemonics: subf
   */
  protected int translateXO_FORM(PPC2IR ppc2ir, PPC_Laziness lazy, int pc,
      int inst, int opcode, int rD, int rA, int rB, int OE,
      int secondaryOpcode, int Rc) {
...
final class mulhw_decoder extends PPC_InstructionDecoder {
  ...
  /**
   * Translate mulhw (multiply high word): <listing> prod[0-63] <- rA * rB rD <-
   * prod </listing>
   */
  protected int translateXO_FORM(PPC2IR ppc2ir, PPC_Laziness lazy, int pc,
      int inst, int opcode, int rD, int rA, int rB, int OE,
...
final class neg_decoder extends PPC_InstructionDecoder {
  ...
  /**
   * Translate neg (negate): <listing> rD <- ¬ (rA) + 1 </listing> Equivalent
   * to: <listing> rD <- -(rA) </listing>
   */
  protected int translateXO_FORM(PPC2IR ppc2ir, PPC_Laziness lazy, int pc,
      int inst, int opcode, int rD, int rA, int rB, int OE,
      int secondaryOpcode, int Rc) {
...
final class subfe_decoder extends PPC_InstructionDecoder {
  ...
  /**
   * Translate subfe (subtract from extended): <listing> rD <- ¬ (rA) + (rB) +
   * XER[CA] </listing> Also modifies XER[CA].
   */
  protected int translateXO_FORM(PPC2IR ppc2ir, PPC_Laziness lazy, int pc,
      int inst, int opcode, int rD, int rA, int rB, int OE,
      int secondaryOpcode, int Rc) {
    ...
    RegisterOperand reg_rB = ppc2ir.getGPRegister(rB);
    RegisterOperand reg_rD = ppc2ir.getGPRegister(rD);
    RegisterOperand tempInt = ppc2ir.getTempInt(0);
    RegisterOperand tempInt2 = ppc2ir.getTempInt(1);

    // tempInt := XER_CA ? 1 : 0
    ppc2ir.appendInstruction(CondMove.create(INT_COND_MOVE,
        tempInt, ppc2ir.getXER_CA_Register(), new IntConstantOperand(0),
        ConditionOperand.EQUAL(), new IntConstantOperand(0),
        new IntConstantOperand(1)));
    // tempInt2 := ¬ (rA)
...
final class adde_decoder extends PPC_InstructionDecoder {
  ...
  /**
   * Translate adde (add extended): <listing> rD <- (rA) + (rB) + XER[CA]
   * </listing> Also modifies XER[CA].
   */
  protected int translateXO_FORM(PPC2IR ppc2ir, PPC_Laziness lazy, int pc,
      int inst, int opcode, int rD, int rA, int rB, int OE,
      int secondaryOpcode, int Rc) {
...
final class subfze_decoder extends PPC_InstructionDecoder {
  ...
  /**
   * Translate subfze (subtract from zero extended): <listing> rD <- ¬(rA) +
   * XER[CA] </listing> Also modifies XER[CA].
   */
  protected int translateXO_FORM(PPC2IR ppc2ir, PPC_Laziness lazy, int pc,
      int inst, int opcode, int rD, int rA, int rB, int OE,
...
final class addze_decoder extends PPC_InstructionDecoder {
  ...
  /**
   * Translate adde (add to zero extended): <listing> rD <- (rA) + XER[CA]
   * </listing> Also modifies XER[CA].
   */
  protected int translateXO_FORM(PPC2IR ppc2ir, PPC_Laziness lazy, int pc,
      int inst, int opcode, int rD, int rA, int rB, int OE,
...
final class addme_decoder extends PPC_InstructionDecoder {
  ...
  /**
   * Translate addme (add to minus one extended): <listing> rD <- (rA) + XER[CA] -
   * 1 </listing> Also modifies XER[CA].
   */
  protected int translateXO_FORM(PPC2IR ppc2ir, PPC_Laziness lazy, int pc,
      int inst, int opcode, int rD, int rA, int rB, int OE,
      int secondaryOpcode, int Rc) {
    ...
    // rD := rD + -1
    ppc2ir.appendInstruction(Binary.create(INT_ADD, reg_rD
        .copyRO(), reg_rD.copyRO(), new IntConstantOperand(0xffffffff)));
    if (Rc != 0) {
      // Set condition codes
      setCRfield(ppc2ir, lazy, 0, reg_rD.copyRO(),
          new IntConstantOperand(0), SIGNED_INT_CMP);
...
final class mullw_decoder extends PPC_InstructionDecoder {
  ...
  /**
   * Translate mullw (multiply low word): <listing> rD <- rA * rB </listing>
   */
  protected int translateXO_FORM(PPC2IR ppc2ir, PPC_Laziness lazy, int pc,
      int inst, int opcode, int rD, int rA, int rB, int OE,
      int secondaryOpcode, int Rc) {
...
final class add_decoder extends PPC_InstructionDecoder {
  ...
  /**
   * Translate add <listing> rD <- rA + rB </listing>
   */
  protected int translateXO_FORM(PPC2IR ppc2ir, PPC_Laziness lazy, int pc,
      int inst, int opcode, int rD, int rA, int rB, int OE,
      int secondaryOpcode, int Rc) {
...
final class divwu_decoder extends PPC_InstructionDecoder {
  ...
  /**
   * Translate divwu (divide word unsigned): <listing> dividend <- (rA) divisor <-
   * (rB) rD <- dividend / divisor </listing>
   */
  protected int translateXO_FORM(PPC2IR ppc2ir, PPC_Laziness lazy, int pc,
      int inst, int opcode, int rD, int rA, int rB, int OE,
      int secondaryOpcode, int Rc) {
...
final class divw_decoder extends PPC_InstructionDecoder {
  ...
  /**
   * Translate divw (divide word) <listing> dividend <- (rA) divisor <- (rB) rD <-
   * dividend / divisor </listing>
   */
  protected int translateXO_FORM(PPC2IR ppc2ir, PPC_Laziness lazy, int pc,
      int inst, int opcode, int rD, int rA, int rB, int OE,
      int secondaryOpcode, int Rc) {
...
final class mtfsf_decoder extends PPC_InstructionDecoder {
  ...
  private void setVX(PPC2IR ppc2ir) {
    ...
    RegisterOperand FPSCR = ppc2ir.getFPSCRRegister();
    // FPSCR = FPSCR & 0b 1101 1111 1111 1111
    ppc2ir.appendInstruction(Binary.create(INT_AND, FPSCR
        .copyRO(), FPSCR, new IntConstantOperand(0xdfffffff)));
    BasicBlock testVX = ppc2ir.getCurrentBlock();
    BasicBlock setVX = ppc2ir.createBlockAfterCurrent();
    BasicBlock end = ppc2ir.createBlockAfter(setVX);

    // Test whether to set VX bit
    {
      // If any one (or more) of a set of bits (VXxxx) is non-zero,
      // the VX bit is set.
      RegisterOperand vxBits = ppc2ir.getTempInt(1);
      testVX.appendInstruction(Binary.create(INT_AND, vxBits, FPSCR.copyRO(),
          new IntConstantOperand(0x1f80700)));

      RegisterOperand guardResult = null;
      testVX.appendInstruction(IfCmp.create(INT_IFCMP, guardResult, vxBits
          .copyRO(), new IntConstantOperand(0), ConditionOperand
          .EQUAL(), end.makeJumpTarget(), BranchProfileOperand.likely()));
      testVX.insertOut(end);
    }
    // Set the VX bit
    {
      setVX.appendInstruction(Binary.create(INT_OR, FPSCR.copyRO(), FPSCR
          .copyRO(), new IntConstantOperand(0x20000000)));
    }
  ...
  protected void setFEX(PPC2IR ppc2ir) {
    ...
    BasicBlock currentBlock = ppc2ir.getCurrentBlock();
    // Clear the FEX bit then test if we need to set it
    RegisterOperand FPSCR = ppc2ir.getFPSCRRegister();
    // FPSCR = FPSCR & 0b 1011 1111 1111 1111
    ppc2ir.appendInstruction(Binary.create(INT_AND, FPSCR
        .copyRO(), FPSCR, new IntConstantOperand(0xbfffffff)));

    // We need to make several tests of the bits of the FPSCR, so
    // make some blocks to do this.
    BasicBlock testVE_OE_UE_ZE_XE = ppc2ir.getCurrentBlock();
    BasicBlock testV = ppc2ir.createBlockAfter(testVE_OE_UE_ZE_XE);
    BasicBlock testO = ppc2ir.createBlockAfter(testV);
    BasicBlock testU = ppc2ir.createBlockAfter(testO);
    BasicBlock testZ = ppc2ir.createBlockAfter(testU);
    BasicBlock testX = ppc2ir.createBlockAfter(testZ);
    // The block to set the FEX bit
    BasicBlock setFEX = ppc2ir.createBlockAfter(testX);
    // The block after everything is done
    BasicBlock end = ppc2ir.createBlockAfter(setFEX);

    // Make a block to set the FEX bit
    {
      // FPSCR = FPSCR | 0b 0100 0000 0000 0000
      setFEX.appendInstruction(Binary.create(INT_OR, FPSCR.copyRO(), FPSCR
          .copyRO(), new IntConstantOperand(0x40000000)));
    }

    // Make a test to see if any exception bits are enabled, if not goto end
    {
      RegisterOperand VE_OE_UE_ZE_XE = ppc2ir.getTempInt(1);
      testVE_OE_UE_ZE_XE.appendInstruction(Binary.create(INT_AND,
          VE_OE_UE_ZE_XE, FPSCR.copyRO(), new IntConstantOperand(0xf8)));
      testVE_OE_UE_ZE_XE.appendInstruction(IfCmp.create(INT_IFCMP, null,
          VE_OE_UE_ZE_XE.copyRO(), new IntConstantOperand(0),
          ConditionOperand.EQUAL(), end.makeJumpTarget(),
    ...
    {
      RegisterOperand tempVX = ppc2ir.getTempInt(1);
      RegisterOperand tempVE = ppc2ir.getTempInt(2);
      RegisterOperand VX = ppc2ir.getTempInt(3);
      RegisterOperand VE = ppc2ir.getTempInt(4);
      RegisterOperand andV = ppc2ir.getTempInt(5);
      // Extract VX bit.
      testV.appendInstruction(Binary.create(INT_AND, tempVX, ppc2ir
          .getFPSCRRegister(), new IntConstantOperand(0x20000000)));
      // Shift to the last bit position.
      testV.appendInstruction(Binary.create(INT_USHR, VX, tempVX.copyRO(),
          new IntConstantOperand(29)));
      // Extract VE bit.
      testV.appendInstruction(Binary.create(INT_AND, tempVE, ppc2ir
          .getFPSCRRegister(), new IntConstantOperand(0x80)));
      // Shift to the last bit position.
      testV.appendInstruction(Binary.create(INT_USHR, VE, tempVE.copyRO(),
          new IntConstantOperand(7)));
      // AND them together.
      testV.appendInstruction(Binary.create(INT_AND, andV, VX.copyRO(), VE
          .copyRO()));
      // If the result is 1, jump to set the FEX bit.
      guardResult = null;
      testV.appendInstruction(IfCmp.create(INT_IFCMP, guardResult, andV,
          new IntConstantOperand(1), ConditionOperand.EQUAL(), setFEX
              .makeJumpTarget(), BranchProfileOperand.unlikely()));
    ...
    {
      RegisterOperand tempOX = ppc2ir.getTempInt(1);
      RegisterOperand tempOE = ppc2ir.getTempInt(2);
      RegisterOperand OX = ppc2ir.getTempInt(3);
      RegisterOperand OE = ppc2ir.getTempInt(4);
      RegisterOperand andO = ppc2ir.getTempInt(5);
      // Extract OX bit.
      testO.appendInstruction(Binary.create(INT_AND, tempOX, ppc2ir
          .getFPSCRRegister(), new IntConstantOperand(0x10000000)));
      // Shift to the last bit position.
      testO.appendInstruction(Binary.create(INT_USHR, OX, tempOX.copyRO(),
          new IntConstantOperand(28)));
      // Extract OE bit.
      testO.appendInstruction(Binary.create(INT_AND, tempOE, ppc2ir
          .getFPSCRRegister(), new IntConstantOperand(0x40)));
      // Shift to the last bit position.
      testO.appendInstruction(Binary.create(INT_USHR, OE, tempOE.copyRO(),
          new IntConstantOperand(6)));
      // AND them together.
      testO.appendInstruction(Binary.create(INT_AND, andO, OX.copyRO(), OE
          .copyRO()));
      // If the result is 1, jump to set the FEX bit.
      guardResult = null;
    ...
    {
      RegisterOperand tempUX = ppc2ir.getTempInt(1);
      RegisterOperand tempUE = ppc2ir.getTempInt(2);
      RegisterOperand UX = ppc2ir.getTempInt(3);
      RegisterOperand UE = ppc2ir.getTempInt(4);
      RegisterOperand andU = ppc2ir.getTempInt(5);
      // Extract UX bit.
      testU.appendInstruction(Binary.create(INT_AND, tempUX, ppc2ir
          .getFPSCRRegister(), new IntConstantOperand(0x8000000)));
      // Shift to the last bit position.
      testU.appendInstruction(Binary.create(INT_USHR, UX, tempUX.copyRO(),
          new IntConstantOperand(27)));
      // Extract UE bit.
      testU.appendInstruction(Binary.create(INT_AND, tempUE, ppc2ir
          .getFPSCRRegister(), new IntConstantOperand(0x20)));
      // Shift to the last bit position.
      testU.appendInstruction(Binary.create(INT_USHR, UE, tempUE.copyRO(),
          new IntConstantOperand(5)));
      // AND them together.
      testU.appendInstruction(Binary.create(INT_AND, andU, UX.copyRO(), UE
          .copyRO()));
      // If the result is 1, jump to set the FEX bit.
      guardResult = null;
    ...
    {
      RegisterOperand tempZX = ppc2ir.getTempInt(1);
      RegisterOperand tempZE = ppc2ir.getTempInt(2);
      RegisterOperand ZX = ppc2ir.getTempInt(3);
      RegisterOperand ZE = ppc2ir.getTempInt(4);
      RegisterOperand andZ = ppc2ir.getTempInt(5);
      // Extract ZX bit.
      testZ.appendInstruction(Binary.create(INT_AND, tempZX, ppc2ir
          .getFPSCRRegister(), new IntConstantOperand(0x4000000)));
      // Shift to the last bit position.
      testZ.appendInstruction(Binary.create(INT_USHR, ZX, tempZX.copyRO(),
          new IntConstantOperand(26)));
      // Extract ZE bit.
      testZ.appendInstruction(Binary.create(INT_AND, tempZE, ppc2ir
          .getFPSCRRegister(), new IntConstantOperand(0x10)));
      // Shift to the last bit position.
      testZ.appendInstruction(Binary.create(INT_USHR, ZE, tempZE.copyRO(),
          new IntConstantOperand(4)));
      // AND them together.
      testZ.appendInstruction(Binary.create(INT_AND, andZ, ZX.copyRO(), ZE
          .copyRO()));
      // If the result is 1, jump to set the FEX bit.
      guardResult = null;
    ...
    {
      RegisterOperand tempXX = ppc2ir.getTempInt(1);
      RegisterOperand tempXE = ppc2ir.getTempInt(2);
      RegisterOperand XX = ppc2ir.getTempInt(3);
      RegisterOperand XE = ppc2ir.getTempInt(4);
      RegisterOperand andX = ppc2ir.getTempInt(5);
      // Extract XX bit.
      testX.appendInstruction(Binary.create(INT_AND, tempXX, ppc2ir
          .getFPSCRRegister(), new IntConstantOperand(0x2000000)));
      // Shift to the last bit position.
      testX.appendInstruction(Binary.create(INT_USHR, XX, tempXX.copyRO(),
          new IntConstantOperand(25)));
      // Extract XE bit.
      testX.appendInstruction(Binary.create(INT_AND, tempXE, ppc2ir
          .getFPSCRRegister(), new IntConstantOperand(0x8)));
      // Shift to the last bit position.
      testX.appendInstruction(Binary.create(INT_USHR, XE, tempXE.copyRO(),
          new IntConstantOperand(3)));
      // AND them together.
      testX.appendInstruction(Binary.create(INT_AND, andX, XX.copyRO(), XE
          .copyRO()));
      // If the result is not 1, jump to avoid setting the FEX bit.
      guardResult = null;
      testX.appendInstruction(IfCmp.create(INT_IFCMP, guardResult, andX,
          new IntConstantOperand(1), ConditionOperand.NOT_EQUAL(), end
              .makeJumpTarget(), BranchProfileOperand.likely()));
   ...
   */
  protected int translateXFL_FORM(PPC2IR ppc2ir, PPC_Laziness lazy, int pc,
      int inst, int opcode, int zero, int FM, int zero2, int frB,
      int secondaryOpcode, int Rc) {
    ...
    if (FMask == 0xffffffff) {
      ...
      ppc2ir.appendInstruction(Binary.create(INT_AND, tempInt
          .copyRO(), tempInt.copyRO(), new IntConstantOperand(FMask)));
      // Calculate FPSCR & NOT mask
      RegisterOperand tempInt2 = ppc2ir.getTempInt(1);
      ppc2ir.appendInstruction(Binary.create(INT_AND, tempInt2,
          ppc2ir.getFPSCRRegister(), new IntConstantOperand(~FMask)));
...
final class mtcrf_decoder extends PPC_InstructionDecoder {
  ...
  /**
   * Translate mtcrf (move to condition register fields) <listing> mask <-
   * (4)(CRM[0]) || (4)(CRM[1]) ||... (4)(CRM[7]) CR <- (rS & mask) | (CR &
   * ¬mask) </listing>
   */
  protected int translateXFX_FORM(PPC2IR ppc2ir, PPC_Laziness lazy, int pc,
      int inst, int opcode, int rS, int CRM0, int secondaryOpcode, int zero) {
    int CRM = CRM0 >> 1;
     ...
     * ppc2ir.getCRRegister(), ppc2ir.getGPRegister(rS))); for(int f=0; f < 8;
     * f++) { lazy.setConditionFieldLaziness(f, PPC_Laziness.NOT_LAZY); } } else {
     * int CRMask = 0; // CR mask (1 bit per bit of the CR)
     *  // CRM has one bit per field of the CR (set to 1 if that field // is to
     * be written to). Make CRMask have 4 identical bits per // field, to
     * correspond to the full 32 bits of the CR. for(int f=0 ; f < 8 ; f++) {
     * if(((CRM >>> (7 - f)) & 0x1) != 0) { CRMask |= (0xf0000000 >>> (4 * f));
     * lazy.setConditionFieldLaziness(f, PPC_Laziness.NOT_LAZY); } } // Set bits
     * in CR register RegisterOperand tempInt = ppc2ir.getTempInt(0);
     * RegisterOperand cr = ppc2ir.getCRRegister();
     * 
     * ppc2ir.appendInstructionToCurrentBlock(Binary.create(INT_AND, cr,
     * cr.copyRO(), new IntConstantOperand(~CRMask)));
     * ppc2ir.appendInstructionToCurrentBlock(Binary.create(INT_AND, tempInt,
     * ppc2ir.getGPRegister(rS), new IntConstantOperand(CRMask)));
     * ppc2ir.appendInstructionToCurrentBlock(Binary.create(INT_OR, cr.copyRO(),
     * tempInt.copyRO(), cr.copyRO())); } return pc + 4;
     */
  }
}
...
final class mfspr_decoder extends PPC_InstructionDecoder {
  ...
  /**
   * Translate mfspr (move from special purpose register) <listing> n <-
   * spr[5-9] || spr[0-4] rS <- SPR(n) </listing>
   */
  protected int translateXFX_FORM(PPC2IR ppc2ir, PPC_Laziness lazy, int pc,
      int inst, int opcode, int rS, int spr, int secondaryOpcode, int zero) {
    spr = ((spr & 0x1f) << 5) | (spr >> 5);

    switch (spr) {
    case 1: // XER
    {
      RegisterOperand XER = ppc2ir.getTempInt(0);
      RegisterOperand XER_SO_Set = ppc2ir.getTempInt(1);
      ppc2ir.appendInstruction(Binary.create(INT_OR, XER_SO_Set,
          ppc2ir.getXER_ByteCountRegister(),
          new IntConstantOperand(1 << 31)));
      ppc2ir.appendInstruction(CondMove.create(INT_COND_MOVE,
          XER, ppc2ir.getXER_SO_Register(), new IntConstantOperand(0),
          ConditionOperand.EQUAL(), ppc2ir.getXER_ByteCountRegister(),
          XER_SO_Set.copyRO()));
      RegisterOperand XER_OV_Set = ppc2ir.getTempInt(1);
      ppc2ir.appendInstruction(Binary.create(INT_OR, XER_OV_Set,
          XER.copyRO(), new IntConstantOperand(1 << 30)));
      ppc2ir.appendInstruction(CondMove.create(INT_COND_MOVE, XER
          .copyRO(), ppc2ir.getXER_OV_Register(),
          new IntConstantOperand(0), ConditionOperand.EQUAL(), XER
              .copyRO(), XER_OV_Set.copyRO()));
      RegisterOperand XER_CA_Set = ppc2ir.getTempInt(1);
      ppc2ir.appendInstruction(Binary.create(INT_OR, XER_OV_Set,
          XER.copyRO(), new IntConstantOperand(1 << 29)));
      ppc2ir.appendInstruction(CondMove.create(INT_COND_MOVE, XER
...
final class mtspr_decoder extends PPC_InstructionDecoder {
  ...
  /**
   * Translate mtspr (move to special purpose register) <listing> n <- spr[5-9] ||
   * spr[0-4] SPR(n) <- rS </listing>
   */
  protected int translateXFX_FORM(PPC2IR ppc2ir, PPC_Laziness lazy, int pc,
      int inst, int opcode, int rS, int spr, int secondaryOpcode, int zero) {
    spr = ((spr & 0x1f) << 5) | (spr >> 5);
...
final class cmp_decoder extends PPC_InstructionDecoder {
  ...
  /**
   * Translate cmp (compare): <listing> if L=0 then a <- EXTS(rA) b <- EXTS(rB)
   * else a <- rA b <- rB if a < b then c <- 0b100 else if a > EXTS(SIMM) then c <-
   * 0b010 else c <- 0b001 CR[(4 * crfD)-(4 * crfD + 3)] <- c || XER[SO]
   * </listing> The L bit has no effect on 32-it operations. Other simplified
   * mnemonics: cmpw
   */
  protected int translateX_FORM(PPC2IR ppc2ir, PPC_Laziness lazy, int pc,
    ...
    setCRfield(ppc2ir, lazy, crfD, ppc2ir.getGPRegister(rA), ppc2ir
        .getGPRegister(rB), SIGNED_INT_CMP);
...
final class mfcr_decoder extends PPC_InstructionDecoder {
  ...
  /**
   * Translate mfcr (move from condition register): <listing> rD <- CR
   * </listing>
   */
  protected int translateX_FORM(PPC2IR ppc2ir, PPC_Laziness lazy, int pc,
...
final class lwarx_decoder extends PPC_InstructionDecoder {
  ...
  /**
   * Translate lwarx (load word and reserve indexed) <listing> if rA = 0 then b <-
   * 0 else b <- (rA) EA <- b + (rB) RESERVE <- 1 RESERVE_ADDR <-
   * physical_addr(EA) rD <- MEM(EA, 4) </listing> See also: stwcx
   */
  protected int translateX_FORM(PPC2IR ppc2ir, PPC_Laziness lazy, int pc,
      int inst, int opcode, int rD, int rA, int rB, int secondaryOpcode,
      int zero) {
    RegisterOperand EA = ppc2ir.getTempInt(0);
...
final class lwzx_decoder extends PPC_InstructionDecoder {
  ...
  /**
   * Translate lwzx (load word and zero indexed) <listing> if rA = 0 then b <- 0
   * else b <- (rA) EA <- b + (rB) rD <- MEM(EA, 4) </listing>
   */
  protected int translateX_FORM(PPC2IR ppc2ir, PPC_Laziness lazy, int pc,
      int inst, int opcode, int rD, int rA, int rB, int secondaryOpcode,
      int zero) {
...
final class slw_decoder extends PPC_InstructionDecoder {
  ...
  /**
   * Translate slw (shift left word) <listing> n <- rB[27-31] r <- ROTL(rS,n)
   * </listing> Equivalent to: <listing> rD <- rA << rB </listing>
   */
  protected int translateX_FORM(PPC2IR ppc2ir, PPC_Laziness lazy, int pc,
...
final class cntlzw_decoder extends PPC_InstructionDecoder {
  ...
  /**
   * Translate cntlzw (count leading zeros word) <listing> n <- 0 do while n <
   * 32 if rS[n] = then leave n <- n + 1 rA <- n </listing>
   * 
   * There are a number of count leading zero algorithms (a good list is at
   * {@url http://www.hackersdelight.org/HDcode/nlz.cc}). This routine
   * delegates to the routine it conceives to be more optimal
   */
  protected int translateX_FORM(PPC2IR ppc2ir, PPC_Laziness lazy, int pc,
   ...
   * <listing> int nlz4(unsigned x) { int y, m, n;
   * 
   * y = -(x >> 16); // If left half of x is 0, m = (y >> 16) & 16; // set n =
   * 16. If left half n = 16 - m; // is nonzero, set n = 0 and x = x >> m; //
   * shift x right 16. // Now x is of the form 0000xxxx. y = x - 0x100; // If
   * positions 8-15 are 0, m = (y >> 16) & 8; // add 8 to n and shift x left 8.
   * n = n + m; x = x << m;
   * 
   * y = x - 0x1000; // If positions 12-15 are 0, m = (y >> 16) & 4; // add 4 to
   * n and shift x left 4. n = n + m; x = x << m;
   * 
   * y = x - 0x4000; // If positions 14-15 are 0, m = (y >> 16) & 2; // add 2 to
   * n and shift x left 2. n = n + m; x = x << m;
   * 
   * y = x >> 14; // Set y = 0, 1, 2, or 3. m = y & ~(y >> 1); // Set m = 0, 1,
   * 2, or 2 resp. return n + 2 - m; } </listing>
   */
  private static void nlz_count_no_branches(PPC2IR ppc2ir, int rS, int rA) {
    RegisterOperand x = ppc2ir.getTempInt(0);
    RegisterOperand y = ppc2ir.getTempInt(1);
    RegisterOperand m = ppc2ir.getTempInt(2);
    RegisterOperand n = ppc2ir.getTempInt(3);
    ...
    ppc2ir.appendInstruction(Binary.create(INT_SUB, y.copyRO(), x
        .copyRO(), new IntConstantOperand(0x100)));
    // m = (y >> 16) & 8; // add 8 to n and shift x left 8.
    ...
    // y = x - 0x1000; // If positions 12-15 are 0,
    ppc2ir.appendInstruction(Binary.create(INT_SUB, y.copyRO(), x
        .copyRO(), new IntConstantOperand(0x1000)));
    // m = (y >> 16) & 4; // add 4 to n and shift x left 4.
    ppc2ir.appendInstruction(Binary.create(INT_SHR, m.copyRO(), y
        .copyRO(), new IntConstantOperand(16)));
    ppc2ir.appendInstruction(Binary.create(INT_AND, m.copyRO(), m
        .copyRO(), new IntConstantOperand(4)));
    // n = n + m;
    ppc2ir.appendInstruction(Binary.create(INT_ADD, n.copyRO(), n
        .copyRO(), m.copyRO()));
    // x = x << m;
    ...
    // y = x - 0x4000; // If positions 14-15 are 0,
    ppc2ir.appendInstruction(Binary.create(INT_SUB, y.copyRO(), x
        .copyRO(), new IntConstantOperand(0x4000)));
    // m = (y >> 16) & 2; // add 2 to n and shift x left 2.
    ppc2ir.appendInstruction(Binary.create(INT_SHR, m.copyRO(), y
        .copyRO(), new IntConstantOperand(16)));
    ppc2ir.appendInstruction(Binary.create(INT_AND, m.copyRO(), m
        .copyRO(), new IntConstantOperand(2)));
    // n = n + m;
    ppc2ir.appendInstruction(Binary.create(INT_ADD, n.copyRO(), n
        .copyRO(), m.copyRO()));
    // x = x << m;
    ...
    ppc2ir.appendInstruction(Binary.create(INT_USHR, y.copyRO(),
        x.copyRO(), new IntConstantOperand(14)));
    // m = y & ~(y >> 1); // Set m = 0, 1, 2, or 2 resp.
    ppc2ir.appendInstruction(Binary.create(INT_SHR, m.copyRO(), y
        .copyRO(), new IntConstantOperand(1)));
...
final class and_decoder extends PPC_InstructionDecoder {
  ...
  /**
   * Translate and <listing> rA <- (rS) & (rB) </listing>
   */
  protected int translateX_FORM(PPC2IR ppc2ir, PPC_Laziness lazy, int pc,
      int inst, int opcode, int rS, int rA, int rB, int secondaryOpcode, int Rc) {
    // Perform and
...
final class cmpl_decoder extends PPC_InstructionDecoder {
  ...
  /**
   * Translate cmpl (compare logical): <listing> a <- (rA) b <- (rB) if a <U b
   * then c <- 0b100 else if a >U ((16)0 || UIMM) then c <- 0b010 else c <-
   * 0b001 CR[(4 * crfD)-(4 * crfD + 3)] <- c || XER[SO] </listing>
   */
  protected int translateX_FORM(PPC2IR ppc2ir, PPC_Laziness lazy, int pc,
      int inst, int opcode, int crfD0L, int rA, int rB, int secondaryOpcode,
      int Rc) {
    int crfD = crfD0L >>> 2;

    setCRfield(ppc2ir, lazy, crfD, ppc2ir.getGPRegister(rA), ppc2ir
        .getGPRegister(rB), UNSIGNED_INT_CMP);
...
final class dcbst_decoder extends PPC_InstructionDecoder {
   ...
   */
  protected int translateX_FORM(PPC2IR ppc2ir, PPC_Laziness lazy, int pc,
      int inst, int opcode, int zero, int rA, int rB, int secondaryOpcode,
      int zero2) {
...
final class andc_decoder extends PPC_InstructionDecoder {
  ...
  /**
   * Translate andc (and with complement) <listing> rA <- (rS) & ¬(rB)
   * </listing>
   */
  protected int translateX_FORM(PPC2IR ppc2ir, PPC_Laziness lazy, int pc,
      int inst, int opcode, int rS, int rA, int rB, int secondaryOpcode, int Rc) {
...
final class lbzx_decoder extends PPC_InstructionDecoder {
  ...
  /**
   * Translate lbz (load byte and zero indexed) <listing> if rA = 0 then b <- 0
   * else b <- (rA) EA <- b + (rB) rD <- (24)0 || MEM(EA, 1) </listing>
   */
  protected int translateX_FORM(PPC2IR ppc2ir, PPC_Laziness lazy, int pc,
      int inst, int opcode, int rD, int rA, int rB, int secondaryOpcode,
      int zero) {
    ...
    ppc2ir.appendInstruction(Binary.create(INT_AND, EA.copyRO(),
        EA.copyRO(), new IntConstantOperand(3)));
    ...
    ppc2ir.appendInstruction(Binary.create(INT_SUB, EA.copyRO(),
        new IntConstantOperand(3), EA.copyRO()));
    ppc2ir.appendInstruction(Binary.create(INT_SHL, EA.copyRO(),
        EA.copyRO(), new IntConstantOperand(3)));
    // -#endif
    // rD >>>= EA
    ...
    // rD &= 0xff
    ppc2ir.appendInstruction(Binary.create(INT_AND, result
        .copyRO(), result.copyRO(), new IntConstantOperand(0xff)));
    return pc + 4;
  }
}
...
final class lbzux_decoder extends PPC_InstructionDecoder {
  ...
  /**
   * Translate lbzux (load byte and zero with update indexed) <listing> if rA =
   * 0 then b <- 0 else b <- (rA) EA <- b + (rB) rD <- (24)0 || MEM(EA, 1)
   * </listing>
   */
  protected int translateX_FORM(PPC2IR ppc2ir, PPC_Laziness lazy, int pc,
      int inst, int opcode, int rD, int rA, int rB, int secondaryOpcode,
...
final class nor_decoder extends PPC_InstructionDecoder {
  ...
  /**
   * Translate nor <listing> rA <- ¬((rS) | (rB)) </listing>
   */
  protected int translateX_FORM(PPC2IR ppc2ir, PPC_Laziness lazy, int pc,
      int inst, int opcode, int rS, int rA, int rB, int secondaryOpcode, int Rc) {
    RegisterOperand tempInt = ppc2ir.getTempInt(0);
...
final class stwcx_decoder extends PPC_InstructionDecoder {
  ...
  /**
   * Translate stwx (store word conditional indexed) <listing> if rA = 0 then b <-
   * 0 else b <- (rA) EA <- b + rB MEM(EA, 4) <- rS if RESERVE then if
   * RESERVE_ADDR = physical_addr(EA) MEM(EA,4) <- rS CR0 <- 0b00 || 0b1 ||
   * XER[SO] else u <- undefined 1-bit value if u then MEM(EA,4) <- rS CR0 <-
   * 0b00 || u || XER[SO] RESERVE <- 0 else CR0 <- 0b00 || 0b0 || XER[SO]
   * </listing> See also: lwarx
   */
  protected int translateX_FORM(PPC2IR ppc2ir, PPC_Laziness lazy, int pc,
      int inst, int opcode, int rS, int rA, int rB, int secondaryOpcode,
      int zero) {
    RegisterOperand EA = ppc2ir.getTempInt(0);
    ...
    setCRfield(ppc2ir, lazy, 0, new IntConstantOperand(0),
        new IntConstantOperand(0), SIGNED_INT_CMP);
    return pc + 4;
  }
}
...
final class stwx_decoder extends PPC_InstructionDecoder {
  ...
  /**
   * Translate stwx (store word indexed) <listing> if rA = 0 then b <- 0 else b <-
   * (rA) EA <- b + rB MEM(EA, 4) <- rS </listing>
   */
  protected int translateX_FORM(PPC2IR ppc2ir, PPC_Laziness lazy, int pc,
      int inst, int opcode, int rS, int rA, int rB, int secondaryOpcode,
      int zero) {
...
final class stwux_decoder extends PPC_InstructionDecoder {
  ...
  /**
   * Translate stwux (store word with update indexed) <listing> if rA = 0 then b <-
   * 0 else b <- (rA) EA <- b + rB MEM(EA, 4) <- rS </listing>
   */
  protected int translateX_FORM(PPC2IR ppc2ir, PPC_Laziness lazy, int pc,
      int inst, int opcode, int rS, int rA, int rB, int secondaryOpcode,
      int zero) {
...
final class stbx_decoder extends PPC_InstructionDecoder {
  ...
  /**
   * Translate stbx (store byte indexed) <listing> if rA = 0 then b <- 0 else b <-
   * (rA) EA <- b + rB MEM(EA, 1) <- rS[24-31] </listing>
   */
  protected int translateX_FORM(PPC2IR ppc2ir, PPC_Laziness lazy, int pc,
      int inst, int opcode, int rS, int rA, int rB, int secondaryOpcode,
      int zero) {
...
final class lhzx_decoder extends PPC_InstructionDecoder {
  ...
  /**
   * Translate lhzx (load half word and zero indexed) <listing> if rA = 0 then b <-
   * 0 else b <- (rA) EA <- b + (rB) rD <- (16)0 || MEM(EA, 2) </listing>
   */
  protected int translateX_FORM(PPC2IR ppc2ir, PPC_Laziness lazy, int pc,
      int inst, int opcode, int rD, int rA, int rB, int secondaryOpcode,
      int zero) {
    ...
    // EA = (EA & 0x2) * 8
    ppc2ir.appendInstruction(Binary.create(INT_AND, EA.copyRO(),
        EA.copyRO(), new IntConstantOperand(0x2)));
    ppc2ir.appendInstruction(Binary.create(INT_SHL, EA.copyRO(),
        EA.copyRO(), new IntConstantOperand(3)));
    // -#else
    // EA = (~EA & 0x2) * 8
    ...
    // rD &= 0xffff
    ppc2ir.appendInstruction(Binary.create(INT_AND, result
        .copyRO(), result.copyRO(), new IntConstantOperand(0xffff)));
    return pc + 4;
  }
}
...
final class xor_decoder extends PPC_InstructionDecoder {
  ...
  /**
   * Translate xor <listing> rA <- (rS) (+) (rB) </listing>
   */
  protected int translateX_FORM(PPC2IR ppc2ir, PPC_Laziness lazy, int pc,
      int inst, int opcode, int rS, int rA, int rB, int secondaryOpcode, int Rc) {
    // Perform xor
...
final class lhax_decoder extends PPC_InstructionDecoder {
  ...
  /**
   * Translate lhax (load half word algabraeic indexed) <listing> if rA = 0 then
   * b <- 0 else b <- (rA) EA <- b+ (rB) rD <- EXTS(MEM(EA, 2)) </listing>
   */
  protected int translateX_FORM(PPC2IR ppc2ir, PPC_Laziness lazy, int pc,
      int inst, int opcode, int rD, int rA, int rB, int secondaryOpcode,
      int zero) {
    ...
    ppc2ir.appendInstruction(Binary.create(INT_SHL, result
        .copyRO(), result.copyRO(), new IntConstantOperand(16)));
    ppc2ir.appendInstruction(Binary.create(INT_SHR, result
        .copyRO(), result.copyRO(), new IntConstantOperand(16)));
    return pc + 4;
  }
...
final class sthx_decoder extends PPC_InstructionDecoder {
  ...
  /**
   * Translate sth (store half word) <listing> if rA = 0 then b <- 0 else b <-
   * (rA) EA <- b+ EXTS(d) MEM(EA, 2) <- rS[16-31] </listing>
   */
  protected int translateX_FORM(PPC2IR ppc2ir, PPC_Laziness lazy, int pc,
      int inst, int opcode, int rS, int rA, int rB, int secondaryOpcode,
      int zero) {
...
final class orc_decoder extends PPC_InstructionDecoder {
  ...
  /**
   * Translate orc (or with complement) <listing> rA <- (rS) | ¬(rB) </listing>
   */
  protected int translateX_FORM(PPC2IR ppc2ir, PPC_Laziness lazy, int pc,
      int inst, int opcode, int rS, int rA, int rB, int secondaryOpcode, int Rc) {
    RegisterOperand tempInt = ppc2ir.getTempInt(0);
...
final class or_decoder extends PPC_InstructionDecoder {
  ...
  /**
   * Translate or <listing> rA <- (rS) | (rB) </listing>
   */
  protected int translateX_FORM(PPC2IR ppc2ir, PPC_Laziness lazy, int pc,
      int inst, int opcode, int rS, int rA, int rB, int secondaryOpcode, int Rc) {
    // Perform or
...
final class lfsx_decoder extends PPC_InstructionDecoder {
  ...
  /**
   * Translate lfsx (load floating-point single indexed) <listing> if rA = 0
   * then b <- 0 else b <- (rA) EA <- b+ (rB) frD <- DOUBLE(MEM(EA, 4))
   * </listing>
   */
  protected int translateX_FORM(PPC2IR ppc2ir, PPC_Laziness lazy, int pc,
      int inst, int opcode, int frD, int rA, int rB, int secondaryOpcode, int Rc) {
    RegisterOperand EA = ppc2ir.getTempInt(0);
    if (rA == 0) {
      // EA = rB
      ppc2ir.appendInstruction(Move.create(INT_MOVE, EA, ppc2ir
...
final class srw_decoder extends PPC_InstructionDecoder {
  ...
  /**
   * Translate srw (shift right word) <listing> n <- rB[27-31] r <-
   * ROTL(rS,32-n) </listing> Equivalent to: <listing> rD <- rA >>> rB
   * </listing>
   */
  protected int translateX_FORM(PPC2IR ppc2ir, PPC_Laziness lazy, int pc,
      int inst, int opcode, int rS, int rA, int rB, int secondaryOpcode, int Rc) {
...
final class lfdx_decoder extends PPC_InstructionDecoder {
  ...
  /**
   * Translate lfdx (load floating-point double indexed) <listing> if rA = 0
   * then b <- 0 else b <- (rA) EA <- b+ (rB) frD <- MEM(EA, 8) </listing>
   */
  protected int translateX_FORM(PPC2IR ppc2ir, PPC_Laziness lazy, int pc,
      int inst, int opcode, int frD, int rA, int rB, int secondaryOpcode,
      int zero) {
    RegisterOperand EA = ppc2ir.getTempInt(0);
    RegisterOperand EA2 = ppc2ir.getTempInt(0 + 1);
    if (rA == 0) {
...
final class stfdx_decoder extends PPC_InstructionDecoder {
  ...
  /**
   * Translate stfdx (store floating-point double indexed) <listing> if rA = 0
   * then b <- 0 else b <- (rA) EA <- b+ (rB) MEM(EA, 8) <- (frS) </listing>
   */
  protected int translateX_FORM(PPC2IR ppc2ir, PPC_Laziness lazy, int pc,
      int inst, int opcode, int frS, int rA, int rB, int secondaryOpcode,
      int zero) {
    RegisterOperand EA = ppc2ir.getTempInt(0);
    RegisterOperand EA2 = ppc2ir.getTempInt(0 + 1);
    if (rA == 0) {
...
final class sraw_decoder extends PPC_InstructionDecoder {
  ...
  /**
   * Translate sraw (shift right algebraic word) <listing> n <- rB[27-31] r <-
   * ROTL(rS,32-n) if rB[26] = 0 then m <- MASK(n) else m <- (32)0 S <- rS rA <-
   * r & m | S & ¬m XER[CA] <- S & ((r & ¬m) != 0) </listing>
   */
  protected int translateX_FORM(PPC2IR ppc2ir, PPC_Laziness lazy, int pc,
      int inst, int opcode, int rS, int rA, int rB, int secondaryOpcode, int Rc) {
    BasicBlock rB26_set = ppc2ir.createBlockAfterCurrent();
    BasicBlock rB26_clear = ppc2ir.createBlockAfter(rB26_set);
    BasicBlock performSraw = ppc2ir.createBlockAfter(rB26_clear);

    // test rB[26]
    RegisterOperand mask = ppc2ir.getTempInt(0);
    ppc2ir.appendInstruction(Binary.create(INT_AND, mask, ppc2ir
        .getGPRegister(rB), new IntConstantOperand(0x10)));
    ppc2ir.appendInstruction(IfCmp.create(INT_IFCMP, null, mask
        .copyRO(), new IntConstantOperand(0), ConditionOperand.EQUAL(),
    ...
    rB26_clear.appendInstruction(Binary.create(INT_SHL, mask.copyRO(),
        new IntConstantOperand(1), ppc2ir.getGPRegister(rB)));
    rB26_clear.appendInstruction(Binary.create(INT_SUB, mask.copyRO(), mask
        .copyRO(), new IntConstantOperand(1)));
    // perform sraw
    ppc2ir.setCurrentBlock(performSraw);
    // shifted bits out of rS
    RegisterOperand shiftedBits = ppc2ir.getTempInt(1);
    ...
    ppc2ir.appendInstruction(BooleanCmp.create(BOOLEAN_CMP_INT,
        ...
        new IntConstantOperand(0), ConditionOperand.EQUAL(),
        BranchProfileOperand.unlikely()));
    if (Rc != 0) {
      // Set condition codes
      setCRfield(ppc2ir, lazy, 0, ppc2ir.getGPRegister(rA),
...
final class srawi_decoder extends PPC_InstructionDecoder {
  ...
  /**
   * Translate srawi (shift right algebraic word immediate) <listing> n <- SH r <-
   * ROTL(rS,32-n) m <- MASK(n) S <- rS rA <- r & m | S & ¬m XER[CA] <- S & ((r &
   * ¬m) != 0) </listing>
   */
  protected int translateX_FORM(PPC2IR ppc2ir, PPC_Laziness lazy, int pc,
    ...
    if (SH == 0) {
      ...
      ppc2ir.appendInstruction(Move.create(LONG_MOVE, ppc2ir
          .getXER_CA_Register(), new LongConstantOperand(0)));
    } else {
      // shifted bits out of rS
      RegisterOperand shiftedBits = ppc2ir.getTempInt(0);
      int mask = (0x1 << SH) - 1;
      ppc2ir.appendInstruction(Binary.create(INT_AND,
          shiftedBits, ppc2ir.getGPRegister(rS), new IntConstantOperand(
              mask)));
      // do rA = rS >> SH
      ppc2ir.appendInstruction(Binary.create(INT_SHR, ppc2ir
          .getGPRegister(rA), ppc2ir.getGPRegister(rS),
          new IntConstantOperand(SH)));
...
final class extsh_decoder extends PPC_InstructionDecoder {
  ...
  /**
   * Translate extsh (extend sign half word) <listing> S <- rS[16] rA[16-31] <-
   * rS[16-31] ra[0-15] <- (16)S </listing>
   */
  protected int translateX_FORM(PPC2IR ppc2ir, PPC_Laziness lazy, int pc,
      int inst, int opcode, int rS, int rA, int zero, int secondaryOpcode,
      int Rc) {
    if (rA == rS) {
      ...
      ppc2ir.appendInstruction(Binary.create(INT_SHL, tempInt,
          ppc2ir.getGPRegister(rS), new IntConstantOperand(16)));
      ppc2ir
          .appendInstruction(Binary.create(INT_SHR, ppc2ir
              .getGPRegister(rA), tempInt.copyRO(), new IntConstantOperand(
              16)));
    }
    if (Rc != 0) {
      // Set condition codes
...
final class fcmpu_decoder extends PPC_InstructionDecoder {
  ...
  /**
   * Translate fcmpu (floating point compare unordered) <listing> if (frA) is a
   * NaN or (frB) is a NaN then c <- 0b0001 else if (frA) < (frB) then c <-
   * 0b1000 else if (frA) > (frB) then c <- 0b0100 else c <- 0b0010 fpscr[FPCC] <-
   * c CR[4 * crfD-4 * crfD + 3] <- c if (frA) is an SNaN or (frB) is an SNaN
   * then fpscr[VXSNAN] <- 1 </listing>
   */
  protected int translateX_FORM(PPC2IR ppc2ir, PPC_Laziness lazy, int pc,
      int inst, int opcode, int crfD00, int frA, int frB, int secondaryOpcode,
      int zero) {
    int crfD = crfD00 >>> 2;

    // Lazily set the condition codes in the CR register
    setCRfield(ppc2ir, lazy, crfD, ppc2ir.getFPRegister(frA), ppc2ir
        .getFPRegister(frB), FP_CMPU);
...
final class mtfsb1_decoder extends PPC_InstructionDecoder {
   ...
   */
  protected int translateX_FORM(PPC2IR ppc2ir, PPC_Laziness lazy, int pc,
      int inst, int opcode, int crbD, int zero, int zero2, int secondaryOpcode,
      int Rc) {
    if ((crbD == 1) || (crbD == 2)) {
...
final class mtfsb0_decoder extends PPC_InstructionDecoder {
  ...
  protected int translateX_FORM(PPC2IR ppc2ir, PPC_Laziness lazy, int pc,
    ...
    ppc2ir.appendInstruction(Binary.create(INT_AND, ppc2ir
        .getFPSCRRegister(), ppc2ir.getFPSCRRegister(),
        new IntConstantOperand(clearBit)));
...
final class mtfsfi_decoder extends PPC_InstructionDecoder {
   ...
   */
  protected int translateX_FORM(PPC2IR ppc2ir, PPC_Laziness lazy, int pc,
      int inst, int opcode, int crfD00, int zero, int imm, int secondaryOpcode,
      int Rc) {
    int crfD = crfD00 >>> 2;
    if (crfD == 0) {
    ...
    ppc2ir.appendInstruction(Binary.create(INT_AND, ppc2ir
        .getFPSCRRegister(), ppc2ir.getFPSCRRegister(),
        new IntConstantOperand(clearBitsMask)));
    ppc2ir.appendInstruction(Binary.create(INT_OR, ppc2ir
        .getFPSCRRegister(), ppc2ir.getFPSCRRegister(),
        new IntConstantOperand(bitsToSet)));
    if (Rc != 0) {
...
final class fnabs_decoder extends PPC_InstructionDecoder {
  ...
  protected int translateX_FORM(PPC2IR ppc2ir, PPC_Laziness lazy, int pc,
      int inst, int opcode, int frD, int zero, int frB, int secondaryOpcode,
      int Rc) {
    BasicBlock fallThrough = ppc2ir.getNextBlock();
    BasicBlock negate = ppc2ir.createBlockAfterCurrent();
    BasicBlock lteZeroTest = ppc2ir.getCurrentBlock();
    ppc2ir.appendInstruction(Move.create(INT_MOVE, ppc2ir
        .getFPRegister(frD), ppc2ir.getFPRegister(frB)));
    lteZeroTest.appendInstruction(IfCmp.create(INT_IFCMP, null, ppc2ir
        .getFPRegister(frB), new DoubleConstantOperand(0),
        ConditionOperand.LESS_EQUAL(), fallThrough.makeJumpTarget(), ppc2ir
            .getConditionalBranchProfileOperand(true)));
    lteZeroTest.insertOut(fallThrough);
    negate.appendInstruction(Unary.create(DOUBLE_NEG,
        ppc2ir.getFPRegister(frD), ppc2ir.getFPRegister(frD)));
    if (Rc != 0) {
...
final class fabs_decoder extends PPC_InstructionDecoder {
  ...
  protected int translateX_FORM(PPC2IR ppc2ir, PPC_Laziness lazy, int pc,
    ...
    BasicBlock fallThrough = ppc2ir.getNextBlock();
    BasicBlock negate = ppc2ir.createBlockAfterCurrent();
    BasicBlock gteZeroTest = ppc2ir.getCurrentBlock();
    ppc2ir.appendInstruction(Move.create(INT_MOVE, ppc2ir
        .getFPRegister(frD), ppc2ir.getFPRegister(frB)));
    gteZeroTest.appendInstruction(IfCmp.create(INT_IFCMP, null, ppc2ir
        .getFPRegister(frB), new DoubleConstantOperand(0),
        ConditionOperand.GREATER_EQUAL(), fallThrough.makeJumpTarget(),
...
final class mffs_decoder extends PPC_InstructionDecoder {
  ...
  public PPC_InstructionDecoder interpretInstruction(PPC_ProcessSpace ps)
    ...
    int frD = bits(ps.currentInstruction, 6, 10);
    int Rc = ps.currentInstruction & 0x1;
    ps.setFPregister(frD, Double
        .longBitsToDouble(((long) ps.fpscr) & 0xFFFFFFFF));
    if (Rc != 0) {
  ...
  /**
   * Translate mffs (move from fpscr) <listing> frD[32-63] <- FPSCR </listing>
   */
  protected int translateX_FORM(PPC2IR ppc2ir, PPC_Laziness lazy, int pc,
      int inst, int opcode, int frD, int zero, int frB, int secondaryOpcode,
      int Rc) {
...
final class rlwimi_decoder extends PPC_InstructionDecoder {
  ...
  /**
   * Translate rlwimi (rotate left word immediate then mask insert) <listing> n <-
   * SH r <- ROTL(rS,n) m <- MASK(MB,ME) rA <- (r & m) | (rA & ¬m) </listing>
   * Other simplified mnemonics: inslwi, inslrwi
   */
  protected int translateM_FORM(PPC2IR ppc2ir, PPC_Laziness lazy, int pc,
      int inst, int opcode, int rS, int rA, int SH, int MB, int ME, int Rc) {
    // Mask has 1 bits from MB to ME, other bits are 0.
    int m = 0;
    // Simple if MB <= ME.
    if (MB <= ME) {
    ...
    if (m == 0) {
      ...
      RegisterOperand tempr = ppc2ir.getTempInt(0);
      if (SH != 0) {
        RegisterOperand tempInt1 = ppc2ir.getTempInt(1);
        RegisterOperand tempInt2 = ppc2ir.getTempInt(2);
        // 1. calculate rS << SH and put into tempInt1
        // 2. calculate rs >>> (32-SH) and put into tempInt2
        // 3. put the bitwise OR of these into tempr (the variable r in
        // the prog. env. manual entry for this instruction).
        ppc2ir.appendInstruction(Binary.create(INT_SHL, tempInt1,
            ppc2ir.getGPRegister(rS), new IntConstantOperand(SH)));
        ppc2ir.appendInstruction(Binary.create(INT_USHR,
            tempInt2, ppc2ir.getGPRegister(rS), new IntConstantOperand(
                32 - SH)));
...
final class rlwinm_decoder extends PPC_InstructionDecoder {
  ...
  /**
   * Translate rlwinm (rotate left word immediate then and with mask) <listing>
   * n <- SH r <- ROTL(rS,n) m <- MASK(MB,ME) rA <- r & m </listing> Other
   * simplified mnemonics: extlwi, extrwi, rotlwi, rotrwi, slwi, srwi, clrlwi,
   * clrrwi, clrlslwi
   */
  protected int translateM_FORM(PPC2IR ppc2ir, PPC_Laziness lazy, int pc,
      int inst, int opcode, int rS, int rA, int SH, int MB, int ME, int Rc) {
    ...
    if (m == 0) {
      ppc2ir.appendInstruction(Move.create(INT_MOVE, ppc2ir
          .getGPRegister(rA), new IntConstantOperand(0)));
    } else {
      RegisterOperand tempr = ppc2ir.getTempInt(0);
      if (SH != 0) {
        RegisterOperand tempInt1 = ppc2ir.getTempInt(1);
      ...
      if (m == 0xffffffff) {
        ...
        // AND the value in tempr with the value m, and put into rA
        ppc2ir.appendInstruction(Binary.create(INT_AND, ppc2ir
            .getGPRegister(rA), tempr.copyRO(), new IntConstantOperand(m)));
      }
    }
    if (Rc != 0) {
      // Set condition codes
...
final class rlwnm_decoder extends PPC_InstructionDecoder {
  ...
  /**
   * Translate rlwnm (rotate left word then and with mask) <listing> n <-
   * rB[27-31] r <- ROTL(rS,n) m <- MASK(MB,ME) rA <- r & m </listing> Other
   * simplified mnemonics: rtolw
   */
  protected int translateM_FORM(PPC2IR ppc2ir, PPC_Laziness lazy, int pc,
      int inst, int opcode, int rS, int rA, int rB, int MB, int ME, int Rc) {
    // Mask has 1 bits from MB to ME, other bits are 0.
    int m = 0;
    // Simple if MB <= ME.
    if (MB <= ME) {
    ...
    if (m == 0) {
      ...
      RegisterOperand tempIntSH = ppc2ir.getTempInt(3);

      ppc2ir.appendInstruction(Binary.create(INT_AND, tempIntSH,
          ppc2ir.getGPRegister(rB), new IntConstantOperand(0x1f)));
      ...
      ppc2ir.appendInstruction(Binary.create(INT_SUB, tempIntSH
          .copyRO(), new IntConstantOperand(32), tempIntSH.copyRO()));
...
final class bc_decoder extends PPC_InstructionDecoder {
  ...
  public PPC_InstructionDecoder interpretInstruction(PPC_ProcessSpace ps)
    ...
    // decode BO
    boolean branch_if_ctr_zero = ((BO & 2) != 0);
    System.out.println("bc(BO=" + BO + ",BI=" + BI + ",BD=" + BD + ",AA=" + AA
        + ",LK=" + LK + ")" + " - target_address=" + target_address + " - ctr="
        + ps.ctr + " - cr=" + ps.getCR());
    if ((BO & 4) == 0) {
      ps.ctr--;
      if ((branch_if_ctr_zero && (ps.ctr != 0))
  ...
  /**
   * Translate bc (branch conditional) <listing> if LK then LR <-iea CIA + 4 if ¬
   * BO[2] then CTR <- CTR - 1 ctr_ok <- BO[2] | ((CTR != 0) (+) BO[3]) cond_ok <-
   * BO[0] | (CR[BI] == BO[1]) if ctr_ok & cond_ok then if AA then NIA <-iea
   * EXTS(BD || 0b00) else NIA <-iea CIA + EXTS(BD || 0b00) </listing> Other
   * simplified mnemonics: bltctr, bnectr
   */
  protected int translateB_FORM(PPC2IR ppc2ir, PPC_Laziness lazy, int pc,
      int inst, int opcode, int BO, int BI, int BD, int AA, int LK) {
    // Translation process:
    ...
    if ((LK == 0) || ppc2ir.traceContinuesAfterBranchAndLink(pc)) {
      ...
      if (BO == 0x14) {
        ...
        if ((target_address > pc) || (found_mapping)
            || (!DBT_Options.optimizeBackwardBranches)) {
          return pc + 4;
        } else {
...
final class b_decoder extends PPC_InstructionDecoder {
  ...
  /**
   * Translate b (branch) <listing> if AA then NIA <-iea EXTS(LI || 0b00) else
   * NIA <- CIA + EXTS(LI || 0b00) if LK then LR <-iea CIA + 4 </listing> Other
   * simplified mnemonics: ba, bl, bla
   */
  protected int translateI_FORM(PPC2IR ppc2ir, PPC_Laziness lazy, int pc,
      int instr, int opcode, int li, int aa, int lk) {
    li = EXTS(li, 24) << 2; // This is what the programming environment manual
                            // denotes as EXTS(LI || 0b00)
    ...
    if (lk != 0) {
      ...
      ppc2ir.appendInstruction(Move.create(INT_MOVE, ppc2ir
          .getLRRegister(), new IntConstantOperand(pc + 4)));
      ...
      if (ppc2ir.traceContinuesAfterBranchAndLink(pc)) {
        ...
        ppc2ir.appendTraceExit(
            ...
            new IntConstantOperand(target_addr));
        return -1;
      }
    } else {
      return target_addr;
...
final class fdivs_decoder extends PPC_InstructionDecoder {
  ...
  protected int translateA_FORM(PPC2IR ppc2ir, PPC_Laziness lazy, int pc,
      ...
      int secondaryOpcode, int Rc) {
    // @todo setting of fpscr
    RegisterOperand tempFloat1 = ppc2ir.getTempFloat(0);
    RegisterOperand tempFloat2 = ppc2ir.getTempFloat(1);
    ppc2ir.appendInstruction(Unary.create(DOUBLE_2FLOAT,
...
final class fmadd_decoder extends PPC_InstructionDecoder {
   ...
   */
  protected int translateA_FORM(PPC2IR ppc2ir, PPC_Laziness lazy, int pc,
      int instr, int opcode, int frD, int frA, int frB, int frC,
      int secondaryOpcode, int Rc) {
    // @todo setting of fpscr
    RegisterOperand tempDouble = ppc2ir.getTempDouble(0);
