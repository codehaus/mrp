...
class X86_ModRM_Decoder extends X86_InstructionDecoder {
   ...
   * 
   * @param override
   *          has the address size been overridden?
  ...
  int displacementSize(boolean override, X86_SIB_Decoder sib) {
    ...
    case 0:
      if (_16BIT || override) {
        ...
        if ((rm == 5) || ((rm == 4) && (sib.getBase(mod) == -1))) {
          return 32;
        } else {
          return 0;
        }
  ...
  X86_DecodedOperand getRM(CodeTranslator translationHelper, Laziness lazy,
    ...
    if (addressSize == 32) {
      ...
      case 0:
        switch (rm) {
        case 0:
          return X86_DecodedOperand.getMemory(segment, X86_Registers.EAX, 0,
              -1, 0, addressSize, operandSize);
        case 1:
          return X86_DecodedOperand.getMemory(segment, X86_Registers.ECX, 0,
              -1, 0, addressSize, operandSize);
        case 2:
          return X86_DecodedOperand.getMemory(segment, X86_Registers.EDX, 0,
              -1, 0, addressSize, operandSize);
        case 3:
          return X86_DecodedOperand.getMemory(segment, X86_Registers.EBX, 0,
              -1, 0, addressSize, operandSize);
        case 4:
          return X86_DecodedOperand.getMemory(segment, sib.getBase(mod), sib
              .getScale(), sib.getIndex(), displacement, addressSize,
              operandSize);
        case 5:
          return X86_DecodedOperand.getMemory(segment, -1, 0, 0, displacement,
              addressSize, operandSize);
        case 6:
          return X86_DecodedOperand.getMemory(segment, X86_Registers.ESI, 0,
              -1, 0, addressSize, operandSize);
        default:
          return X86_DecodedOperand.getMemory(segment, X86_Registers.EDI, 0,
              -1, 0, addressSize, operandSize);
        }
      case 1:
      case 2:
        ...
        case 0:
          return X86_DecodedOperand.getMemory(segment, X86_Registers.EAX, 0,
              -1, displacement, addressSize, operandSize);
        case 1:
          return X86_DecodedOperand.getMemory(segment, X86_Registers.ECX, 0,
              -1, displacement, addressSize, operandSize);
        case 2:
        ...
        case 4:
          return X86_DecodedOperand.getMemory(segment, sib.getBase(mod), sib
              ...
              operandSize);
        case 5:
          return X86_DecodedOperand.getMemory(segment, X86_Registers.EBP, 0,
              -1, displacement, addressSize, operandSize);
        case 6:
          return X86_DecodedOperand.getMemory(segment, X86_Registers.ESI, 0,
              -1, displacement, addressSize, operandSize);
  ...
  /**
   * Get the reg field as a decoded operand, the reg field encodes either a
   * register or an opcode
   */
  ...
  /**
   * Disassemble the RM field as a decoded operand, the RM field encodes either
   * a register or memory location
   */
  String disassembleRM(X86_SIB_Decoder sib, int displacement, int operandSize,
      int addressSize, int seg) {
    ...
    if (addressSize == 32) {
      ...
      case 0: // no displacement, rm==4 or rm==5 are special
        ...
        case 3:
          return segment + "[EBX]";
        case 4:
          return segment + "[" + sib.disassemble(mod) + "]";
        case 5:
          return segment + "[0x" + Integer.toHexString(displacement) + "]";
        case 6:
          return segment + "[ESI]";
        default:
          return segment + "[EDI]";
      ...
      case 2: // 32bit displacement
        switch (rm) {
        case 0:
          return segment + "[EAX+0x" + Integer.toHexString(displacement) + "]";
        case 1:
          return segment + "[ECX+0x" + Integer.toHexString(displacement) + "]";
        case 2:
          return segment + "[EDX+0x" + Integer.toHexString(displacement) + "]";
        case 3:
          return segment + "[EBX+0x" + Integer.toHexString(displacement) + "]";
        case 4:
          return segment + "[" + sib.disassemble(mod) + "+0x"
              + Integer.toHexString(displacement) + "]";
        case 5:
          return segment + "[EBP+0x" + Integer.toHexString(displacement) + "]";
        case 6:
          return segment + "[ESI+0x" + Integer.toHexString(displacement) + "]";
        default:
          return segment + "[EDI+0x" + Integer.toHexString(displacement) + "]";
        }
      default: // Case 3, the RM field just encodes a direct register access
      ...
      case 0: // no displacement and rm==6 is special
        switch (rm) {
        case 0:
          return segment + "[BX+SI]";
        case 1:
          return segment + "[BX+DI]";
        case 2:
          return segment + "[BP+SI]";
        case 3:
          return segment + "[BP+DI]";
        case 4:
          return segment + "[SI]";
        case 5:
          return segment + "[DI]";
      ...
      case 2: // 16bit displacement
        switch (rm) {
        case 0:
          return segment + "[BX+SI+0x" + Integer.toHexString(displacement)
              + "]";
        case 1:
          return segment + "[BX+DI+0x" + Integer.toHexString(displacement)
              + "]";
        case 2:
          return segment + "[BP+SI+0x" + Integer.toHexString(displacement)
              + "]";
        case 3:
          return segment + "[BP+DI+0x" + Integer.toHexString(displacement)
              + "]";
        case 4:
          return segment + "[SI+0x" + Integer.toHexString(displacement) + "]";
        case 5:
          return segment + "[DI+0x" + Integer.toHexString(displacement) + "]";
        case 6:
          return segment + "[BP+0x" + Integer.toHexString(displacement) + "]";
        default:
          return segment + "[BX+0x" + Integer.toHexString(displacement) + "]";
        }
      default: // Case 3, the RM field just encodes a direct register access
        return X86_Registers.disassemble(rm, operandSize);
      }
  ...
  /**
   * Disassemble the reg field as a decoded operand, the reg field encodes
