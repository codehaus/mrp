...
public class VM_NormalMethod extends VM_Method implements VM_BytecodeConstants {
  ...
  // TODO: rework the system so we don't waste space for this on the VM_Method
  // object
  ...
  /*
   * prologue may change the maximum stack height, remember the original stack
   * height
   */
   ...
   * 
   * @param dc
   *          the VM_TypeReference object of the class that declared this field
   * @param mr
   ...
   * @param lw
   *          the number of local words used by the bytecode of this method
   * @param ow
   *          the number of operand words used by the bytecode of this method
   * @param bc
   *          the bytecodes of this method
   * @param eMap
   *          the exception handler map for this method
   * @param lm
   *          the line number map for this method
   * @param constantPool
   *          the constantPool for this method
   * @param sig
   *          generic type of this method.
   * @param annotations
   *          array of runtime visible annotations
   * @param parameterAnnotations
   *          array of runtime visible paramter annotations
   * @param ad
   *          annotation default value for that appears in annotation classes
   */
  public VM_NormalMethod(VM_TypeReference dc, VM_MemberReference mr, short mo,
      VM_TypeReference[] et, short lw, short ow, byte[] bc,
      VM_ExceptionHandlerMap eMap, int[] lm, int[] constantPool, VM_Atom sig,
  ...
  /**
   * Space required by this method for its local variables, in words. Note:
  ...
  public void getDynamicLink(VM_DynamicLink dynamicLink, int bcIndex) {
    if (VM.VerifyAssertions)
      VM._assert(bytecodes != null);
    if (VM.VerifyAssertions)
      VM._assert(bcIndex + 2 < bytecodes.length);
    ...
    int constantPoolIndex = ((bytecodes[bcIndex + 1] & 0xFF) << BITS_IN_BYTE)
        | (bytecodes[bcIndex + 2] & 0xFF);
    dynamicLink.set(getDeclaringClass().getMethodRef(constantPoolIndex),
        bytecode);
  }
  ...
  public int getLineNumberForBCIndex(int bci) {
    ...
    for (idx = 0; idx < lineNumberMap.length; idx++) {
      ...
      if (bci < pc) {
        if (idx == 0)
          idx++; // add 1, so we can subtract 1 below.
        break;
      }
    }
  ...
  /**
   * Sets method in state for OSR specialization, i.e, the subsequent calls of
   * {@link #getBytecodes} return the stream of specialized bytecodes.
   * 
   * NB: between flag and action, it should not allow GC or threadSwitch happen.
   * 
   * @param prologue
   *          The bytecode of prologue
   * @param newStackHeight
   *          The prologue may change the default height of stack
   */
   ...
   * 
   * @return the length of prologue if the method is in state for OSR, 0
   *         otherwise.
   */
  ...
  public VM_BytecodeStream getOsrPrologue() {
    if (VM.VerifyAssertions)
      VM._assert(synthesizedBytecodes != null);
   ...
   *         that will be generated by the opt compiler if the method is
   *         inlined.
   */
  ...
  /**
   * Set the value of the 'runtime service method' flag to the argument value. A
   * method is considered to be a runtime service method if it is
   * only/primarialy invoked "under the covers" from the generated code and thus
   * is not subject to inlining via the normal mechanisms. For example, the
   * implementations of bytecodes such as new or checkcast or the implementation
   * of yieldpoints.
   * 
   * @param value
   *          true if this is a runtime service method, false it is not.
   */
  ...
  /**
   * This method computes a summary of interesting method characteristics and
   * stores an encoding of the summary as an int.
   */
  protected void computeSummary(int[] constantPool) {
    ...
    while (bcodes.hasMoreBytecodes()) {
      ...
      case JBC_ifnonnull:
        ...
        if (bcodes.getBranchOffset() < 0)
          summaryFlags |= HAS_BACK_BRANCH;
        calleeSize += SIMPLE_OPERATION_COST;
        continue; // we've processed all of the bytes, so avoid the call to
                  // skipInstruction()
      case JBC_goto:
        if (bcodes.getBranchOffset() < 0)
          summaryFlags |= HAS_BACK_BRANCH;
        ...
        continue; // we've processed all of the bytes, so avoid the call to
                  // skipInstruction()
      case JBC_goto_w:
        if (bcodes.getWideBranchOffset() < 0)
          summaryFlags |= HAS_BACK_BRANCH;
        calleeSize += SIMPLE_OPERATION_COST;
        continue; // we've processed all of the bytes, so avoid the call to
                  // skipInstruction()
