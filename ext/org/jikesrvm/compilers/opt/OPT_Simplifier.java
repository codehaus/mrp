...
public abstract class OPT_Simplifier extends OPT_IRTools {
      ...
      if (op2.isConstant() && !op2.isObjectConstant()) {
        ...
        if (op1.isConstant() && !op1.isObjectConstant()) {
          ...
          if (val2.isZero()) { // x + 0 == x
            if (op1.isIntLike()) {
              Unary.mutate(s, INT_2ADDRSigExt, Binary.getClearResult(s), Binary
                  .getClearVal1(s));
              return DefUseEffect.REDUCED;
            } else {
              Move.mutate(s, REF_MOVE, Binary.getClearResult(s), Binary
                  .getClearVal1(s));
              return op1.isConstant() ? DefUseEffect.MOVE_FOLDED
                  : DefUseEffect.MOVE_REDUCED;
            }
    ...
    if (CF_ADDR) {
      ...
      if (op2.isAddressConstant()) {
        ...
        if (op1.isAddressConstant()) {
          ...
          Move.mutate(s, REF_MOVE, Binary.getClearResult(s), AC(val1.and(val2)
              .toAddress()));
          return DefUseEffect.MOVE_FOLDED;
        } else {
          // ONLY OP2 IS CONSTANT: ATTEMPT TO APPLY AXIOMS
          if (val2.isZero()) { // x & 0 == 0
            Move.mutate(s, REF_MOVE, Binary.getClearResult(s), AC(Address
                .zero()));
            return DefUseEffect.MOVE_FOLDED;
          }
    ...
    if (CF_ADDR) {
      ...
      if (op2.isIntConstant()) {
        ...
        if (op1.isAddressConstant()) {
          // BOTH CONSTANTS: FOLD
          Word val1 = op1.asAddressConstant().value.toWord();
          Move.mutate(s, REF_MOVE, Binary.getClearResult(s), AC(val1.lsh(val2)
              .toAddress()));
          return DefUseEffect.MOVE_FOLDED;
        } else {
          // ONLY OP2 IS CONSTANT: ATTEMPT TO APPLY AXIOMS
    ...
    if (CF_ADDR) {
      ...
      if (op2.isIntConstant()) {
        ...
        if (op1.isAddressConstant()) {
          // BOTH CONSTANTS: FOLD
          Word val1 = op1.asAddressConstant().value.toWord();
          Move.mutate(s, REF_MOVE, Binary.getClearResult(s), AC(val1.rsha(val2)
              .toAddress()));
          return DefUseEffect.MOVE_FOLDED;
        } else {
          // ONLY OP2 IS CONSTANT: ATTEMPT TO APPLY AXIOMS
  ...
  private static DefUseEffect refNot(OPT_Instruction s) {
    if (CF_ADDR) {
      ...
      if (op.isAddressConstant()) {
        ...
        Move.mutate(s, REF_MOVE, Unary.getClearResult(s), AC(val.not()
            .toAddress()));
        return DefUseEffect.MOVE_FOLDED;
      }
    }
    ...
    if (CF_ADDR) {
      ...
      if (op2.isAddressConstant()) {
        ...
        if (op1.isAddressConstant()) {
          // BOTH CONSTANTS: FOLD
          Word val1 = op1.asAddressConstant().value.toWord();
          Move.mutate(s, REF_MOVE, Binary.getClearResult(s), AC(val1.or(val2)
              .toAddress()));
          return DefUseEffect.MOVE_FOLDED;
        } else {
          // ONLY OP2 IS CONSTANT: ATTEMPT TO APPLY AXIOMS
    ...
    if (CF_ADDR) {
      ...
      if (op2.isConstant() && !op2.isObjectConstant()) {
        ...
        if (op1.isConstant() && !op1.isObjectConstant()) {
          // BOTH CONSTANTS: FOLD
          Address val1 = getAddressValue(op1);
          Move.mutate(s, REF_MOVE, Binary.getClearResult(s), AC(val1.minus(val2
              .toWord().toOffset())));
          return DefUseEffect.MOVE_FOLDED;
        } else {
          // ONLY OP2 IS CONSTANT: ATTEMPT TO APPLY AXIOMS
          ...
          // x - c = x + -c
          // prefer adds, since some architectures have addi but not subi
          Binary.mutate(s, REF_ADD, Binary.getClearResult(s), Binary
              .getClearVal1(s), AC(Address.zero().minus(
              val2.toWord().toOffset())));
          return DefUseEffect.REDUCED;
        }
      }
    }
    ...
    if (CF_ADDR) {
      ...
      if (op2.isIntConstant()) {
        ...
        if (op1.isAddressConstant()) {
          // BOTH CONSTANTS: FOLD
          Word val1 = op1.asAddressConstant().value.toWord();
          Move.mutate(s, REF_MOVE, Binary.getClearResult(s), AC(val1.rshl(val2)
              .toAddress()));
          return DefUseEffect.MOVE_FOLDED;
        } else {
          // ONLY OP2 IS CONSTANT: ATTEMPT TO APPLY AXIOMS
    ...
    if (CF_ADDR) {
      ...
      if (op2.isAddressConstant()) {
        ...
        if (op1.isAddressConstant()) {
          // BOTH CONSTANTS: FOLD
          Word val1 = op1.asAddressConstant().value.toWord();
          Move.mutate(s, REF_MOVE, Binary.getClearResult(s), AC(val1.xor(val2)
              .toAddress()));
          return DefUseEffect.MOVE_FOLDED;
        } else {
          // ONLY OP2 IS CONSTANT: ATTEMPT TO APPLY AXIOMS
          if (val2.isMax()) { // x ^ -1 == x ^ 0xffffffff = ~x
            Unary.mutate(s, REF_NOT, Binary.getClearResult(s), Binary
                .getClearVal1(s));
            return DefUseEffect.REDUCED;
          }
          if (val2.isZero()) { // x ^ 0 == x
    ...
    if (CF_LONG) {
      ...
      if (op2.isLongConstant()) {
        ...
        if (op1.isLongConstant()) {
          ...
          // ONLY OP2 IS CONSTANT: ATTEMPT TO APPLY AXIOMS
          if (val2 == 0L) { // x + 0 == x
            Move.mutate(s, LONG_MOVE, Binary.getClearResult(s), Binary
                .getClearVal1(s));
            return DefUseEffect.MOVE_REDUCED;
          }
        }
    ...
    if (CF_LONG) {
      ...
      if (op2.isLongConstant()) {
        ...
        if (op1.isLongConstant()) {
          // BOTH CONSTANTS: FOLD
          long val1 = op1.asLongConstant().value;
          Move.mutate(s, LONG_MOVE, GuardedBinary.getClearResult(s), LC(val1
              / val2));
          return DefUseEffect.MOVE_FOLDED;
        } else {
          // ONLY OP2 IS CONSTANT: ATTEMPT TO APPLY AXIOMS
    ...
    if (CF_LONG) {
      ...
      if (op2.isLongConstant()) {
        ...
        if (op1.isLongConstant()) {
          ...
          // ONLY OP2 IS CONSTANT: ATTEMPT TO APPLY AXIOMS
          if (val2 == -1L) { // x * -1 == -x
            Move.mutate(s, LONG_NEG, Binary.getClearResult(s), Binary
                .getClearVal1(s));
            return DefUseEffect.REDUCED;
          }
          if (val2 == 0L) { // x * 0L == 0L
      ...
      if (op2.isLongConstant()) {
        ...
        if (op1.isLongConstant()) {
          ...
          if (VM.BuildFor64Addr) {
            // x - c = x + -c
            // prefer adds, since some architectures have addi but not subi
            Binary.mutate(s, LONG_ADD, Binary.getClearResult(s), Binary
                .getClearVal1(s), LC(-val2));
            return DefUseEffect.REDUCED;
          }
        }
      }
    ...
    if (CF_LONG) {
      ...
      if (op2.isIntConstant()) {
        ...
        if (op1.isLongConstant()) {
          ...
          long val1 = op1.asLongConstant().value;
          Move
              .mutate(s, LONG_MOVE, Binary.getClearResult(s), LC(val1 >>> val2));
          return DefUseEffect.MOVE_FOLDED;
        } else {
          // ONLY OP2 IS CONSTANT: ATTEMPT TO APPLY AXIOMS
          if (val2 == 0) { // x >>> 0L == x
      ...
      if (op2.isLongConstant()) {
        ...
        if (op1.isLongConstant()) {
          ...
          if (val2 == -1L) { // x ^ -1L == x ^ 0xff..ff = ~x
            Unary.mutate(s, LONG_NOT, Binary.getClearResult(s), Binary
                .getClearVal1(s));
            return DefUseEffect.REDUCED;
          }
          if (val2 == 0L) { // x ^ 0L == x
    ...
    if (CF_FLOAT) {
      ...
      if (op2.isFloatConstant()) {
        ...
        if (val2 == 0.0f) {
          ...
          Move.mutate(s, FLOAT_MOVE, Binary.getClearResult(s), Binary
              .getClearVal1(s));
          return DefUseEffect.MOVE_REDUCED;
        }
      }
    ...
    if (CF_DOUBLE) {
      ...
      if (op2.isDoubleConstant()) {
        ...
        if (op1.isDoubleConstant()) {
          ...
          Move
              .mutate(s, DOUBLE_MOVE, Binary.getClearResult(s), DC(val1 + val2));
          return DefUseEffect.MOVE_FOLDED;
        }
        if (val2 == 0.0) {
          // x + 0.0 is x, even if x is a NaN
          Move.mutate(s, DOUBLE_MOVE, Binary.getClearResult(s), Binary
              .getClearVal1(s));
          return DefUseEffect.MOVE_REDUCED;
        }
      }
  ...
  private static DefUseEffect doubleAsLongBits(OPT_Instruction s) {
    if (CF_LONG) {
      ...
      if (op.isDoubleConstant()) {
        // CONSTANT: FOLD
        double val = op.asDoubleConstant().value;
        Move.mutate(s, LONG_MOVE, Unary.getClearResult(s), LC(Double
  ...
  private static DefUseEffect int2AddrSigExt(OPT_Instruction s) {
    if (CF_ADDR) {
      ...
      if (op.isIntConstant()) {
        // CONSTANT: FOLD
        int val = op.asIntConstant().value;
        Move.mutate(s, REF_MOVE, Unary.getClearResult(s), AC(Address
            .fromIntSignExtend(val)));
        return DefUseEffect.MOVE_FOLDED;
      }
    }
    return DefUseEffect.UNCHANGED;
  ...
  private static DefUseEffect intBitsAsFloat(OPT_Instruction s) {
    if (CF_FLOAT) {
      ...
      if (op.isIntConstant()) {
        // CONSTANT: FOLD
        int val = op.asIntConstant().value;
        Move.mutate(s, FLOAT_MOVE, Unary.getClearResult(s), FC(Float
            .intBitsToFloat(val)));
        return DefUseEffect.MOVE_FOLDED;
      }
    }
  ...
  private static DefUseEffect floatAsIntBits(OPT_Instruction s) {
    if (CF_INT) {
      ...
      if (op.isFloatConstant()) {
        // CONSTANT: FOLD
        float val = op.asFloatConstant().value;
        Move.mutate(s, INT_MOVE, Unary.getClearResult(s), IC(Float
            .floatToIntBits(val)));
        return DefUseEffect.MOVE_FOLDED;
      }
    }
  ...
  private static DefUseEffect longBitsAsDouble(OPT_Instruction s) {
    if (CF_DOUBLE) {
      ...
      if (op.isLongConstant()) {
        // CONSTANT: FOLD
        long val = op.asLongConstant().value;
        Move.mutate(s, DOUBLE_MOVE, Unary.getClearResult(s), DC(Double
            .longBitsToDouble(val)));
        return DefUseEffect.MOVE_FOLDED;
      }
    }
    return DefUseEffect.UNCHANGED;
  ...
  private static DefUseEffect boundsCheck(OPT_Instruction s) {
    if (CF_FIELDS) {
      ...
      if (ref.isNullConstant()) {
        ...
        if (indexAsInt < 0) {
          Trap.mutate(s, TRAP, BoundsCheck.getClearGuardResult(s),
          ...
          if (indexAsInt < refLength) {
            ...
            return Move.getVal(s).isConstant() ? DefUseEffect.MOVE_FOLDED
                : DefUseEffect.MOVE_REDUCED;
          } else {
            Trap.mutate(s, TRAP, BoundsCheck.getClearGuardResult(s),
                OPT_TrapCodeOperand.ArrayBounds());
  ...
  private static DefUseEffect getField(OPT_Instruction s) {
    if (CF_FIELDS) {
      ...
      if (ref.isNullConstant()) {
        ...
        try {
          OPT_ConstantOperand op = OPT_StaticFieldReader
              .getFieldValueAsConstant(field, ref.asObjectConstant().value);
          Move.mutate(s, OPT_IRTools.getMoveOp(field.getType()), GetField
              .getClearResult(s), op);
          return DefUseEffect.MOVE_FOLDED;
      ...
      if (op.isNullConstant()) {
        ...
        try {
          ...
          if (VM.runningVM) {
            ...
            // only valid in the bootstrap JVM
            System.out.println("Failed to resolve: " + op.getType() + ": "
                + e.getMessage());
          }
        }
      } else {
  ...
  private static DefUseEffect getClassTib(OPT_Instruction s) {
    if (CF_TIB) {
      ...
      if (typeOp.getTypeRef().isResolved()) {
        Move.mutate(s, REF_MOVE, Unary.getClearResult(s),
  ...
  private static DefUseEffect getSuperclassIdsFromTib(OPT_Instruction s) {
    if (CF_TIB) {
      ...
      if (tibOp.isTIBConstant()) {
        OPT_TIBConstantOperand tib = tibOp.asTIBConstant();
        Move.mutate(s, REF_MOVE, Load.getClearResult(s),
            new OPT_ObjectConstantOperand(tib.value.getSuperclassIds(), Offset
                .zero()));
        return DefUseEffect.MOVE_FOLDED;
      }
    }
  ...
  private static DefUseEffect getDoesImplementFromTib(OPT_Instruction s) {
    if (CF_TIB) {
      ...
      if (tibOp.isTIBConstant()) {
        OPT_TIBConstantOperand tib = tibOp.asTIBConstant();
        Move.mutate(s, REF_MOVE, Load.getClearResult(s),
            new OPT_ObjectConstantOperand(tib.value.getDoesImplement(), Offset
                .zero()));
        return DefUseEffect.MOVE_FOLDED;
      }
    }
        ...
        if (tib.value.isInstantiated() && offset.isConstant()) {
          ...
          if (tib.value.isTIBSlotTIB(intSlot)) {
            ...
            if (tibArray[intSlot] == null) {
              ...
              result = new OPT_ObjectConstantOperand(tibArray[intSlot], Offset
                  .zero());
            }
          }
  ...
  /**
   * To reduce the number of conditions to consider, we transform all
   * commutative operators to a canoncial form. The following forms are
