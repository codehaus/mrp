...
public abstract class JNICompiler implements BaselineConstants {
  ...
  public static synchronized CompiledMethod compile(NativeMethod method) {
    ...
    //   ForwardReference fr = asm.forwardJcc(AssemblerConstants.LGE);
    //   asm.emitINT_Imm(3);
    //   fr.resolve(asm);
  ...
  public static void generateGlueCodeForJNIMethod(Assembler asm, NormalMethod method, int methodID) {
    ...
    if (VM.VerifyAssertions) VM._assert(stackDepth << LG_WORDSIZE == STACKFRAME_BODY_OFFSET.toInt() - (SAVED_GPRS_FOR_JNI << LG_WORDSIZE), "of2fp="+stackDepth+" sg4j="+SAVED_GPRS_FOR_JNI);
    ...
    ForwardReference doneLeaveJNIRef = asm.forwardJcc(AssemblerConstants.EQ);
  ...
  public static void generateEpilogForJNIMethod(Assembler asm, RVMMethod method) {
    ...
    ForwardReference doneEnterJNIRef = asm.forwardJcc(AssemblerConstants.EQ);
