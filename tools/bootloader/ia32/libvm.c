...
/**
 * These vars help implement the two-phase trap handler approach for
 * alignment checking
 */
static unsigned int alignCheckHandlerJumpLocation = 0;
/* ought to be enough to hold two instructions */
static unsigned char alignCheckHandlerInstBuf[100];

/**
 * if enabled, print a character for each alignment trap (whether or
 * not we ignore it)
 */
static int alignCheckVerbose = 0;

/** statistics defined in sysAlignmentCheck.c */
...
int inRVMAddressSpace(Address addr)
{
  int which;
  /* get the boot record */
 ...
 * alignment error
 * @return true iff the trap handler should return immediately
 */
int handleAlignmentTrap(int signo, void* context) {
  if (signo == SIGBUS) {
    /* first turn off alignment checks for the handler so we can don't
       cause another one here */
	...
	TRACE_PRINTF("N"); // native code, apparently
      }
      ignore = 1;
      numNativeAlignTraps++;
    } else {
    ...
    if (ignore) {
      ...
      alignCheckHandlerJumpLocation = localInstructionAddress;
  ...
  // alignment checking: handle the second phase of align traps that
  // the code above decided to ignore
...
static void hardwareTrapHandler(int signo, siginfo_t *si, void *context)
{
  ...
  if (isVmSignal(localInstructionAddress, localNativeThreadAddress))
    {
      if (lib_verbose) TRACE_PRINTF("it's a VM signal.\n");
  ...
  if (*vmr_inuse || !isRecoverable) {
    if (*vmr_inuse)
      TRACE_PRINTF("%s: internal error: recursive use of"
  ...
  *(Address *)(threadObjectAddress + RVMThread_stackLimit_offset) = stackLimit;
  ...
  if (lib_verbose)
    TRACE_PRINTF("Trap code is 0x%x\n", IA32_EAX(context));
  ...
  *(Address*)vmr_fp = localFrameAddress;
  ...
  if (lib_verbose)
    TRACE_PRINTF("Set vmr_fp to 0x%x\n", localFrameAddress);
  ...
  *(Address*) (localNativeThreadAddress + Thread_framePointer_offset) = fp;
  ...
  if (lib_verbose)
    TRACE_PRINTF("exiting normally; the context will take care of the rest (or so we hope)\n");
}
...
mapImageFile(const char *fileName, const void *targetAddress, int prot,
  ...
  if (!fin) {
    TRACE_PRINTF("%s: can't find bootimage file\"%s\"\n", Me, fileName);
    return 0;
  }

  /* measure image size */
  if (lib_verbose)
    TRACE_PRINTF("%s: loading from \"%s\"\n", Me, fileName);
...
createVM(int UNUSED vmInSeparateThread)
{
  ...
  if (lib_verbose) {
    ...
    TRACE_PRINTF("   bootImageDataStart:   0x%08x\n",
	    bootRecord->bootImageDataStart);
    TRACE_PRINTF("   bootImageDataEnd:     0x%08x\n",
	    bootRecord->bootImageDataEnd);
    TRACE_PRINTF("   bootImageCodeStart:   0x%08x\n",
	    bootRecord->bootImageCodeStart);
    TRACE_PRINTF("   bootImageCodeEnd:     0x%08x\n",
	    bootRecord->bootImageCodeEnd);
    TRACE_PRINTF("   bootImageRMapStart:   0x%08x\n",
	    bootRecord->bootImageRMapStart);
    TRACE_PRINTF("   bootImageRMapEnd:     0x%08x\n",
	    bootRecord->bootImageRMapEnd);
    TRACE_PRINTF("   initialHeapSize:      0x%08x\n",
	    bootRecord->initialHeapSize);
    TRACE_PRINTF("   maximumHeapSize:      0x%08x\n",
	    bootRecord->maximumHeapSize);
    TRACE_PRINTF("   tiRegister:           0x%08x\n",
	    bootRecord->tiRegister);
    TRACE_PRINTF("   spRegister:           0x%08x\n",
	    bootRecord->spRegister);
    TRACE_PRINTF("   ipRegister:           0x%08x\n",
	    bootRecord->ipRegister);
    TRACE_PRINTF("   tocRegister:          0x%08x\n",
	    bootRecord->tocRegister);
    TRACE_PRINTF("   sysConsoleWriteCharIP:0x%08x\n",
