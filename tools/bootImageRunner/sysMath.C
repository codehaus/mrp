...
{
  SYS_START();
  TRACE_PRINTF(SysTraceFile, "%s: sysLongDivide %ld / %ld\n", Me, a, b);  
  return a/b;
}
...
{
  SYS_START();
  TRACE_PRINTF(SysTraceFile, "%s: sysLongRemainder %ld %% %ld\n", Me, a, b);
  return a % b;
}
...
{
  SYS_START();
  TRACE_PRINTF(SysTraceFile, "%s: sysLongToDouble %ld\n", Me, a);
  return (double)a;
}
...
{
  SYS_START();
  TRACE_PRINTF(SysTraceFile, "%s: sysLongToFloat %ld\n", Me, a);
  return (float)a;
}
